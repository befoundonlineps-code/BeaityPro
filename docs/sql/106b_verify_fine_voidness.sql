...
--
-- ⚠️ **ولو أُضيف `contype` يومًا فليكن `con.contype::text`** — نوعُه `"char"`
-- لا `text`، **وتسلسلُه نصًّا يرفع `operator is not unique`.** وقع عند المراجع.
-- **وهذا الملفُّ لا يستعمله**، فالسطرُ وقايةٌ لمن يزيد.
--
...
select
  ...
  -- 🔴 **الثلاثةُ التالية أضافها المراجعُ بعد أن نفّذ الـview فعلًا** — صفٌّ في
  -- `stock_fines` بلا مقابلٍ في الحيويّة أعاد `is_void` عدمًا **بصمتٍ تامّ**.
  -- ⇒ **فما كان يُفترض في الترويسة صار يُقاس هنا.**

  -- ⚠️ **وعدُّ الغراماتِ اليتيمةِ كان رابعًا هنا وانتقل إلى `106c`** — لأنه
  --   **يقرأ الجدولين فعلًا**، وpostgres يحلّ أسماءَ العلاقات وقتَ التحليل.
  --   🔴 **فغيابُ `stock_document_liveness` كان يُسقط هذا الملفَّ كلَّه بـ`42P01`**
  --   — وهو **الرمزُ الذي تتجنّبه الترويسةُ أعلاه عمدًا** — **ويمنع طبعَ
  --   `liveness_exists = 0`، أي الشاهدِ الذي يقول «هذا هو السبب» بعينه.**
  --   ⇒ **ففُصل بالنوع:** هذا فحصُ شكلٍ وصلاحيّة (كتالوجٌ خالص، ينحدر لطيفًا
  --   دائمًا)، و`106c` فحصُ بيانات. **وسقوطُه هناك خبرٌ لا كارثة.**
  ...
  -- 🔴 **و`NOT NULL` ملصقةٌ بكلّ عمود، وذلك ليس تزيينًا:** الشرطُ الثالثُ
  --   الحاملُ لحجّة ١٠٦ هو `stock_fines.document_id not null` — وبدونه تكون
  --   المقارنةُ غيرَ محقَّقةٍ أبدًا فيعود `is_void` عدمًا.
  --   ⚠️ **و`not null` لا يظهر في `pg_constraint` إطلاقًا على postgres**
  --   (قاسه المراجع: قيدان فقط، `f` و`p`)، **فالعمودُ الثالثُ أعلاه أعمى**
  --   **عنه بنيويًّا** — و`attnotnull` هو موضعُه الوحيد.
  (select string_agg(a.attname || case when a.attnotnull then ' NOT NULL' else '' end,
                     ' · ' order by a.attnum)
     from pg_attribute a
     join pg_class c on c.oid = a.attrelid
     join pg_namespace n on n.oid = c.relnamespace
