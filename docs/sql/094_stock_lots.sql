...
-- ⚠️ يُقرأ التصحيحُ بآخر الملفّ قبل تصديق أيّ جملةٍ عن المِنَح أعلاه.
...
-- ==========================================================================
-- ⚠️ أُضيف بعد التنفيذ. **ولا جملةَ SQL واحدةٍ فوق هذا السطر تغيّرت** — الملفُّ
-- أعلاه هو بالحرف ما لُصق ونُفِّذ، لأن سكربتًا نُفِّذ هو السجلُّ الوحيدُ لما نُفِّذ.
-- (نفسُ ما فعله ٠٥٣أ حين انكشف الادّعاءُ نفسُه فيه.)
--
-- 🔴 جملةٌ فوق **خاطئة**، والمالكُ كشفها بقراءة المِنَح
--
-- كُتب عند المنحة: «ولا `update` ولا `delete` في المنحة أصلًا… وغيابُ الاثنين
-- معًا يجعل تعديلَ دفعةٍ **غيرَ ممكنٍ من طريقين لا واحد**».
--
-- **وهذا صحيحٌ عمّا يفعله هذا السكربت، وخاطئٌ عن الحالة التي يتركها** — وهو
-- بالضبط الفرقُ الذي يُقرأ خطأً. `GRANT` **يضيف ولا يسحب**: امتيازاتُ Supabase
-- الافتراضيّةُ على مخطّط `public` تمنح `anon` و`authenticated` **السبعَ صلاحيّاتٍ
-- على كلّ جدولٍ جديدٍ لحظةَ إنشائه**، فمنحُنا الصريحُ كان إضافةً على موجود.
--
-- **والمقيسُ عند المالك: السبعُ للاثنين، بما فيها UPDATE وDELETE.**
--
-- ⚠️ **وليس هذا اكتشافًا — هو ADR-054، مسجَّلٌ منذ ٠٥٣ب:** «`anon` يملك السبعَ
-- صلاحيّاتٍ على **كلّ** جداول `public` بلا استثناء — **٤١ جدولًا، مقيسًا** —
-- فـRLS ليست خطَّ الدفاع الأوّل بل **الوحيد**».
--
-- 🔴 **والأسوأ أن الملفَّ المرافقَ لهذا يقولها صحيحةً:** ترويسةُ
-- `094b_c_isolation.sql` تنصّ على أن وجودَ `anon` «متوقَّعٌ ولا يُقرأ ثغرة… وRLS
-- هي الطبقةُ الوحيدة هنا لا الثانية، وهذا مسجَّلٌ بـADR-054». **فملفّان في
-- إيداعٍ واحدٍ يتناقضان**، وأحدُهما نُفِّذ.
--
-- ⚠️ **وإجراءٌ موصوفٌ عندنا لهذه الحالة بالذات لم يُشغَّل:** `DATABASE_DIAGRAM`
-- تقول إن **استعلامَ ٠٥٣ج الثالثَ هو التدقيقُ الذي يُعاد بعد كلّ جدولٍ جديد، لا
-- مرّةً واحدة**. أُنشئ جدولٌ جديدٌ ولم يُعَد. **والمالكُ أعاده فعليًّا حين قرأ
-- المِنَح** — فالإجراءُ عمل، ومَن نفّذه ليس مَن كان يجب.
--
-- ---------------------------------------------------------------------------
-- والحالةُ الصحيحةُ الآن، بعد القياس
--
-- **طبقةٌ واحدةٌ لا طبقتان:** RLS وحدَها تمنع تعديلَ دفعةٍ أو حذفَها — إذ لا
-- سياسةَ `update` ولا `delete` عليها، **و`pg_policies` هو ما يجب أن يُقرأ**، لا
-- `role_table_grants`.
--
-- ⚠️ **والمنعُ ما زال قائمًا وفعّالًا**: RLS ترفض أيَّ أمرٍ لا سياسةَ له، فغيابُ
-- السياستين **حظرٌ بنيويّ**. الذي سقط هو ادّعاءُ «طبقتين»، لا الحماية.
--
-- ⇒ **ولم يُضَف `REVOKE`**، وذلك قرارٌ لا إغفال: النمطُ يشمل ٤١ جدولًا ومسجَّلٌ
-- بـADR، **وجدولٌ واحدٌ يخالفها يصير الاستثناءَ الذي يُعاد اشتقاقُ سببه** — وهي
-- عينُ علّة «مختلفٌ لكن مكافئٌ لأسبابٍ مكتوبةٍ في تعليق». **وتشديدُ المِنَح قرارٌ
-- على مستوى المشروع كلِّه، لا على جدولٍ بُني اليوم.**
-- ==========================================================================
