...
erDiagram
    ...
    salons ||--o{ employees : "موظفو الصالون — ⚠️ مرتبطون بـprofiles اختياريًّا، لا مستقلّين"
    profiles ||--o| employees : "employees.profile_id → profiles(id) · UNIQUE · ON DELETE SET NULL"
...
erDiagram
    ...
    storages {
        ...
        boolean is_active
        integer sort_order
        timestamptz created_at "⚠️ كان ناقصًا من هذا السرد — انكشف لمّا select * رجّع 16 عمودًا والسرد فيه 15. ولا updated_at، فما في جواب لـ«إيمتى تغيّر هالصفّ؟»"
    ...
    storage_responsibles {
        ...
        employee_role role
        storage_kind storage_kind "✅ منفَّذٌ بالقاعدة — مقيسٌ بـ٠٧٥_٢ (المفتاحان المركَّبان قائمان مع CHECK). ⚠️ وكان مكتوبًا هنا «⏳ بانتظار تشغيل المالك» وهو حالةُ تشغيلٍ سُجّلت مرّةً ولم تُحدَّث — واستُشهد بها لتفسير غيابٍ محتمل بينما هو موجود. قيمته ثابتة 'common' عمدًا — نصف مفتاح أجنبي على storages(id, kind) يجعل «لا مسؤولين لمستودع مهني» قيدًا بنيويًا لا صمتًا بالواجهة. وسلوكه عند الحذف يطابق سلوك storage_id لا أكثر: مفتاح مرآة وظيفته الصحّة لا دورة الحياة، ومفتاحان يختلفان عليها يجعل الأشدّ يقرّر والأضعف يكذب على قارئه"
...
⚠️ **وثالثةُ الصنف نفسه، مسجَّلةٌ هنا لأن الاثنتين فوقها لم تكفيا:** سُجِّل أن `entered_uom` **بلا حارسٍ بالقاعدة**، لأن السؤال جرى على `pg_constraint` بـ`contype = 'c'` — **والحارس نوعٌ لا قيد.** فـ`stock_movements.entered_uom` من نوع **`entry_uom`**، وهو `enum` حقيقيّ (`typtype = 'e'`) بثلاثة حروفٍ بالضبط: `package` · `portion` · `unit` — **مقيسًا بـ[053c](sql/053c-uom-type-and-rls-exposure.sql)، وستّ دوالّ بالمجلّد كانت تكتبه `(v_line->>'entered_uom')::entry_uom` قبل أن يُسأل.** **والعمود يقبل `NULL`، والفراغُ حصريًّا من `stocktake`** (صفران، ولا صفَّ توريدٍ ولا شطبٍ ولا إرجاعٍ ولا تحويل) — **لأن الجرد يكتب فرقًا محسوبًا لا وحدةً كتبها إنسان.**

---

### `product_orders` · `product_order_lines` — طلبيّة البضاعة (المرحلة ٥)

**⚠️ وهما أوّل جدولين بهذا المستودع يوجد سكربتُ إنشائهما كاملًا** — [053a](sql/053a-product-orders.sql) — **بخلاف كلّ ما فوق، الذي أُنشئ بمحرّر SQL مباشرةً فلا نصَّ له.** فما يلي ليس استنتاجًا من دالّة ولا من فحص PostgREST: **هو التعريف نفسه، متحقَّقًا منه بـ[053b](sql/053b-verify-product-orders.sql) و[053e](sql/053e-verify-order-lines-uom-type.sql).**

```
product_orders       id · salon_id → salons · supplier_id → suppliers
                     order_date (default current_date) · note · created_at
                     unique (id, salon_id)   ← هدفُ المفتاح المركَّب أدناه، لا تكرارٌ للمفتاح الأساسيّ

product_order_lines  id · salon_id · order_id · product_id → products
                     entered_quantity numeric (> 0) · entered_uom entry_uom NOT NULL
                     entered_unit_price numeric(14,4) · sort_order · created_at
                     foreign key (order_id, salon_id) → product_orders (id, salon_id) ON DELETE CASCADE
```

**⚠️ ولماذا `salon_id` على جدول الأسطر:** `stock_movements` هو **الجدول التفصيليّ الوحيد بهذا المخطّط بنفس علاقة الابن بأبيه**، ويحمل عموده الخاصّ بسياسةٍ عليه — **لا اشتقاقًا بربطٍ بالأب.** ونسخُ العمود يفتح ما لا يفتحه الاشتقاق: **صفّان يتناقضان.** فالمفتاح المركَّب هو ما يمنعه — **إذ فحصُ التكامل المرجعيّ يتجاوز RLS دائمًا**، فبدونه يعلّق عميلٌ سطرًا بطلبيّةٍ لا يستطيع رؤيتها.

**⚠️ و`entered_uom` هنا `NOT NULL` بينما هو على `stock_movements` يقبل الفراغ — عن قصد:** سطر الطلبيّة يكتبه إنسانٌ بوحدةٍ اختارها **دائمًا**، ولا صفَّ بشكل الجرد بهذا الجدول.

**⚠️ وسياسة `DELETE` موجودة على الاثنين، وهي الوحيدة بهذا المخطّط** (`products`/`stock_documents`/`stock_movements` بلا واحدة، والحذف يُرفَض بنيويًّا). **والسبب عن البيانات لا عن الاتّساق:** المستند يسجّل حركةً ومالًا فحذفه يمحو سجلّ واقعة، **والطلبيّة لا تسجّل شيئًا وقع ولا شيء يشير إليها** — فلا `from_order_id` إطلاقًا، **ونفس الطلبيّة تُستعمل لتوريدين.**

**⚠️ وهذان الجدولان يُكتبان من العميل مباشرةً عبر PostgREST، لا عبر دالّة** — بخلاف `stock_documents`. **فـ`WITH CHECK` ليس خطّ دفاعٍ ثانيًا بل الوحيد**، و**لا حاجة لنمط `(v_line->>'entered_uom')::entry_uom`** الذي تستعمله الدوالّ الستّ: **PostgREST يحوّل نصّ JSON إلى النوع بنفسه**، والقيمةُ الخاطئة تُرفَض باسم النوع.

**وعن المِنَح و`anon`: راجع ADR-054** — `anon` يملك السبع صلاحيات على **كلّ** جداول `public` بلا استثناء (٤١ جدولًا، مقيسًا)، **فـRLS ليست خطّ الدفاع الأوّل بل الوحيد**، و[053c](sql/053c-uom-type-and-rls-exposure.sql) استعلامُه الثالث هو التدقيق الذي يُعاد بعد **كلّ** جدولٍ جديد.
