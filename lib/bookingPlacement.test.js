...
describe('evaluatePlacement', () => {
  ...
  it('excludes the moved booking from its own resource unit too, not just the employee check', () => {
    // Same nudge as the employee-side drag test, but the service is
    // resource-linked. The unit the booking already holds must not count
    // as busy against the very booking it belongs to.
    const self = booked('being-moved', '10:00', '11:00', 'u1')
    const result = evaluatePlacement({
      ...base,
      start: D('10:30'),
      end: D('11:30'),
      orderedUnits: units,
      unitAppointments: [self],
      excludeAppointmentId: 'being-moved',
    })
    expect(result.ok).toBe(true)
    expect(result.candidateUnits.map((u) => u.id)).toEqual(['u1', 'u2'])
...
describe('combineGroupPlacement', () => {
  const ok = (over) => ({ ok: true, outsideSchedule: false, candidateUnits: [null], ...over })
  const entry = (key, plan, over) => ({ key, employeeName: key, isPrimary: false, plan, ...over })

  it('approves a session where everyone fits, naming nobody as outside', () => {
    const result = combineGroupPlacement([
      entry('sara', ok(), { isPrimary: true }),
      entry('layla', ok()),
    ])
    expect(result).toEqual({ ok: true, outsideKeys: [] })
  })

  it('reports the primary refusal even when a participant also fails', () => {
    // No point naming the helper when the main professional is the blocker.
    const result = combineGroupPlacement([
      entry('sara', { ok: false, reason: 'conflict' }, { isPrimary: true }),
      entry('layla', { ok: false, reason: 'roleMismatch' }),
    ])
    expect(result.ok).toBe(false)
    expect(result.reason).toBe('conflict')
    expect(result.failed.isPrimary).toBe(true)
  })

  it('names the participant when only the participant fails', () => {
    const result = combineGroupPlacement([
      entry('sara', ok(), { isPrimary: true }),
      entry('layla', { ok: false, reason: 'roleMismatch' }),
    ])
    expect(result.ok).toBe(false)
    expect(result.reason).toBe('roleMismatch')
    expect(result.failed.employeeName).toBe('layla')
    expect(result.failed.isPrimary).toBe(false)
  })

  it('collects only the people actually outside their shift', () => {
    // The ordinary mixed case: the main professional is on shift, the
    // helper is being asked to stay late.
    const result = combineGroupPlacement([
      entry('sara', ok(), { isPrimary: true }),
      entry('layla', ok({ outsideSchedule: true })),
    ])
    expect(result).toEqual({ ok: true, outsideKeys: ['layla'] })
  })

  it('collects the primary too when it is the one outside', () => {
    const result = combineGroupPlacement([
      entry('sara', ok({ outsideSchedule: true }), { isPrimary: true }),
      entry('layla', ok()),
    ])
    expect(result).toEqual({ ok: true, outsideKeys: ['sara'] })
  })

  it('handles a lone primary, which is what every ordinary booking is', () => {
    expect(combineGroupPlacement([entry('sara', ok(), { isPrimary: true })])).toEqual({ ok: true, outsideKeys: [] })
  })

  it('tolerates empty input', () => {
    expect(combineGroupPlacement([])).toEqual({ ok: true, outsideKeys: [] })
    expect(combineGroupPlacement(null)).toEqual({ ok: true, outsideKeys: [] })
  })
})
