import {
  ...
  countUoms, defaultCountUom, droppedCounts,
} from './stocktakeSheet'
...
describe('the folder narrows the sheet, never the document', () => {
  // ⚠️ MEASURED BEFORE IT WAS FIXED: counting the hair shelf, switching the
  // filter to nails, counting that too and saving sent ONE line. The other
  // count was discarded without a word, the confirmation said "1 line counted"
  // where the person counted two, and post_stocktake stores no counts (item 44)
  // — so the work was gone with nothing to recover it from.
  //
  // Silent, plausible, permanent: the three properties this module treats as
  // the worst class there is.
  const counts = { 'p-stocked': '7', 'p-nails': '9' }

  it('sends every counted row, whatever folder is on screen', () => {
    const wholeStorage = rowsFor({})
    expect(countedRowsToSend(wholeStorage, counts, {}).map((r) => r.productId).sort())
      .toEqual(['p-nails', 'p-stocked'])
  })

  it('drops a count when handed the narrowed set — which is why the screen is not', () => {
    // The old behaviour, kept as a case so the reason the screen passes the
    // whole storage is written down as an executable fact rather than a comment
    // somebody deletes while tidying.
    const nailsOnly = rowsFor({ categoryId: 'c-nails' })
    expect(countedRowsToSend(nailsOnly, counts, {}).map((r) => r.productId))
      .toEqual(['p-nails'])
  })

  it('names the counts that would be lost, so saving can refuse instead', () => {
    // ⚠️ The fail-closed layer. The scope fix means this list is always empty
    // today; it exists for the day something else narrows the rows — a product
    // archived out of the sheet mid-count, a filter nobody anticipated.
    expect(droppedCounts(rowsFor({ categoryId: 'c-nails' }), counts)).toEqual(['p-stocked'])
    expect(droppedCounts(rowsFor({}), counts)).toEqual([])
  })

  it('does not call an untouched box a lost count', () => {
    // An empty field outside the current folder is not work anybody did, and
    // refusing to save because of one would make the guard noise.
    expect(droppedCounts(rowsFor({ categoryId: 'c-nails' }), { 'p-stocked': '', 'p-nails': '9' }))
      .toEqual([])
    // But a written zero IS work: it says the shelf is empty.
    expect(droppedCounts(rowsFor({ categoryId: 'c-nails' }), { 'p-stocked': '0' }))
      .toEqual(['p-stocked'])
  })

  it('counts the whole document in the summary, not the visible part', () => {
    // Standing in the nail folder, the total still says two lines counted — so
    // nothing the person did has quietly stopped existing on screen either.
    const readings = (rows) => Object.fromEntries(rows.map((r) => {
      const raw = counts[r.product.id]
      return [r.product.id, lineReading(r, raw, countState(raw) === COUNT_STATE.UNTOUCHED ? null : Number(raw))]
    }))
    const whole = rowsFor({})
    expect(stocktakeSummary(whole, readings(whole)).countedLines).toBe(2)
  })
})
