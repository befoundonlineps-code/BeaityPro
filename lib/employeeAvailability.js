...
// The hours this employee works on that one day instead of the pattern, if
// somebody set them by hand.
//
// Rows come from employee_day_hours, at most one per employee per day.
export function dayHoursForDate(dayHours, date) {
  const dateKey = toDateKey(date)
  const row = (dayHours || []).find((h) => h.work_date === dateKey)
  if (!row) return null
...
// coming in.
//
// Hand-set hours for one day replace the pattern rather than joining it.
// That is the whole point of them: added as another window they could only
// ever lengthen the day, and "today she works 11:00–15:00 instead of
// 09:00–18:00" would be unsayable. Exceptions still add on top, because each
// one exists to keep a confirmed booking inside an open window and that stays
// true whatever the shift was changed to.
//
// So the day is decided in three layers, in this order: the pattern or its
// override, plus the exceptions, and then the absence vetoes the lot. Every
// caller gets all three by asking one function, which is why they arrive here
// as arguments rather than being folded in at each call site.
export function availableWindowsForDate(schedule, slots, exceptions, date, absences, dayHours) {
  if (isAbsentOn(absences, date)) return []

  const windows = []
  const base = dayHoursForDate(dayHours, date) || recurringWindowForDate(schedule, slots, date)
  if (base) windows.push(base)
