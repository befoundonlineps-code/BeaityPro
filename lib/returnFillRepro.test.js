...
import { isInteractive } from './interactiveShapes'
...
describe('إعادةُ إنتاج البلاغ — الملءُ يصل الجدول', () => {
  ...
  it('🔴 كلُّ شاشةٍ تحمل مبلغًا تنادي حارسَ المال — لا تكتب شرطَه', () => {
    ...
    // 🔴 **«يكتب فيها إنسان» شرطٌ يُقاس، لا جملةٌ تُقال — وهذا تصويبُ مدًى بعد
    // عضّةٍ على كودٍ سليم.**
    //
    // `ReturnDocumentView` تعرض المبلغَ المستلَم **فتذكر `paidAmount` مفتاحًا
    // في نداء `paymentChoiceOf`** — ولا يكتب فيها أحدٌ شيئًا، **ولا يمكنه:**
    // `viewScreensAreInert` يثبت بالحقن أن لا `<input>` ولا `NumberField` ولا
    // `onChange` ولا نداءَ ترحيلٍ في ذلك المجلّد.
    //
    // ⚠️ **والاستثناءُ بالمجلّد كان سيكون ثقبًا بلا قاع** — شاشةُ عرضٍ تكتب
    // يومًا تمرّ لأن مجلّدَها مستثنًى. **فالشرطُ من النصّ:** ملفٌّ لا يقبل
    // إدخالًا ولا يرسل **ليس شاشةً يكتب فيها إنسان**، أينما كان.
    //
    // ⚠️ **والقائمةُ مستوردةٌ لا منسوخة** (`lib/interactiveShapes.js`) — ونسخةٌ
    // ثانيةٌ منها هي فرصةٌ ثانيةٌ لتتباعد، وهو صنفٌ مكتوبٌ بأربعة أمثلة.
    const mentionsPaid = scanned.filter((f) => /\bpaidAmount\b/.test(strip(fs.readFileSync(f, 'utf8'))))
    const withPaid = mentionsPaid.filter((f) => isInteractive(fs.readFileSync(f, 'utf8')))

    // مشيةٌ لا تجد شيئًا لا تجد مخالفةً أيضًا.
    expect(withPaid.length).toBeGreaterThan(0)

    // 🔴 **والتضييقُ نفسُه مقيسٌ بالاتّجاهين، وإلّا صار تخفيفًا بلبوس تصويب:**
    // ما استُبعد يُسمّى، **وكلُّ مستبعَدٍ يجب أن يكون غيرَ تفاعليٍّ فعلًا.**
    for (const file of mentionsPaid.filter((f) => !withPaid.includes(f))) {
      const name = path.basename(file)
      expect(`${name} ⟵ استُبعد وهو غيرُ تفاعليّ: ${!isInteractive(fs.readFileSync(file, 'utf8'))}`)
        .toBe(`${name} ⟵ استُبعد وهو غيرُ تفاعليّ: true`)
    }
