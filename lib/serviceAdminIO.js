...
import { BUCKET } from './servicePhotos'
...
// Taking the bytes of a picture nothing points at any more.
//
// Every path carries Date.now(), so replacing a picture writes a new object
// and leaves the old one behind — and removing one leaves it behind with no
// column pointing at it at all. Neither is rare: they happen on every single
// change of a picture, so the bucket fills with files no row can name.
//
// Always called after the column has been updated, never before. The other
// order risks deleting the bytes and then failing to update the row, which
// leaves a service pointing at a picture that no longer exists — visibly
// broken, where an orphan is merely wasteful.
export async function deleteServicePhoto(storagePath, client = supabase) {
  // A service that never had a picture has nothing to clean up, and asking
  // storage to remove '' is a request with no meaning.
