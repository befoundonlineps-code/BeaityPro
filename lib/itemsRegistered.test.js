...
// Numbers matched by the pattern that are not project items: file -> number ->
// WHY.
//
// ⚠️ THE REASON IS DATA, NOT A TRAILING COMMENT, and the difference is the whole
// point of this file. The first draft was `file -> [numbers]` with the reason in
// a `//` beside it — and a comment is exactly what nothing enforces. In two
// years that list would be pairs of numbers nobody can justify or remove: the
// fault this guard exists to prevent, recursed into the guard's own escape
// hatch.
//
// A reason held as a value cannot be dropped while the exception stays, because
// dropping it drops the entry.
//
// ⚠️ EMPTY TODAY, and that is a measurement: every "item N" now in the codebase
// is a real citation. Kept as a door rather than cut when needed, because the
// alternative to an easy door is somebody widening the pattern under deadline.
const NOT_PROJECT_ITEMS = {
  // 'lib/example.js': { 404: 'an HTTP status in a cache comment, not an item' },
}
...
function citations() {
  ...
  for (const file of sourceFiles()) {
    ...
    const excused = Object.keys(NOT_PROJECT_ITEMS[relative] || {})
...
describe('the pattern is wide on purpose, and the width is named', () => {
  ...
  it('carries no dead exception', () => {
    ...
    for (const [file, reasons] of Object.entries(NOT_PROJECT_ITEMS)) {
      const full = path.join(ROOT, file)
      expect(fs.existsSync(full)).toBe(true)
      const text = toLatin(fs.readFileSync(full, 'utf8'))
      for (const number of Object.keys(reasons)) {
...
describe('an exception carries its reason as data', () => {
  // ⚠️ THE RECURSION THIS WHOLE FILE EXISTS TO PREVENT, pointed at the file's own
  // escape hatch. The first shape was `file -> [numbers]` with the justification
  // in a `//` beside it — and a comment is exactly what nothing enforces. In two
  // years that becomes a list of numbers nobody can justify or delete: an item
  // living where nobody reads it, inside the guard against items living where
  // nobody reads them.
  //
  // Held as a value, the reason cannot be dropped while the exception stays.
  it('rejects an entry that says only which number to ignore', () => {
    const withoutReason = { 'lib/a.js': { 404: '' } }
    const withReason = { 'lib/a.js': { 404: 'an HTTP status in a cache comment, not an item' } }
    const unjustified = (list) => Object.entries(list).flatMap(([file, reasons]) => (
      Object.entries(reasons)
        .filter(([, why]) => String(why || '').trim().length < 15)
        .map(([number]) => `${file}:${number}`)
    ))

    expect(unjustified(withoutReason)).toEqual(['lib/a.js:404'])
    expect(unjustified(withReason)).toEqual([])
    // And the real list, whatever it holds today or later.
    expect(unjustified(NOT_PROJECT_ITEMS)).toEqual([])
  })

  it('will not accept a bare array, which is the shape that lost the reason', () => {
    // Object.keys of an array gives indices, so the old shape would silently
    // excuse '0' and '1' rather than 404 — failing OPEN on numbers nobody named
    // and leaving the real one unexcused. Worth a test, because the migration
    // between the two shapes is exactly where that would slip through.
    expect(Object.keys(['404', '2'])).toEqual(['0', '1'])
    for (const reasons of Object.values(NOT_PROJECT_ITEMS)) {
      expect(Array.isArray(reasons)).toBe(false)
    }
  })
})
