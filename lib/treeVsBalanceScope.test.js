...
const cat = (id, storage_id) => ({
  id, name: id, parent_id: null, sort_order: 1, is_active: true,
})

const COSMO = 'stor-cosmo'
const HAIR = 'stor-hair'

const CATEGORIES = [cat('after-laser'), cat('bath')]

// الانتماءُ صفوفٌ في storage_categories. والمجلّدُ يقدر يكون بأكتر من مستودع،
// والرصيدُ ما بيسمع عنه إطلاقًا — وهاد بالضبط الفرقُ اللي بيحرسه هالملفّ.
const LINKS = [
  { id: 'l1', storage_id: COSMO, category_id: 'after-laser' },
  { id: 'l2', storage_id: HAIR, category_id: 'bath' },
]
...
describe('the tree narrows with the storage — this is the new behaviour', () => {
  it('shows one folder here and a different one there', () => {
    expect(foldersForStorage(CATEGORIES, COSMO, LINKS).map((c) => c.id)).toEqual(['after-laser'])
    expect(foldersForStorage(CATEGORIES, HAIR, LINKS).map((c) => c.id)).toEqual(['bath'])
  })

  it('shows both under «all storages»', () => {
    expect(foldersForStorage(CATEGORIES, ALL_STORAGES, LINKS)).toHaveLength(2)
  })
})
...
describe('the balance does NOT narrow with the folder — this must never change', () => {
  ...
  it('ignores a folder handed to it, rather than merely not being handed one', () => {
    // 🔴 THIS IS THE ASSERTION THAT ACTUALLY BIT, AND ITS FIRST NAME UNDERSOLD
    // IT. The forbidden change was injected — three lines teaching
    // catalogueBalanceRows to filter products by `categoryId` — and the two
    // behavioural tests above went on passing, because neither of them PASSES a
    // folder. A caller that did would have got 0 instead of 60.
    //
    // ⚠️ So «the balance does not narrow with the folder» cannot be checked by
    // calling it without a folder. It has to be called WITH one and shown to
    // ignore it. That is the difference between «my cases all pass» and «the
    // case that would fail passes too».
    const merged = catalogueBalanceRows({
      balances: BALANCES, products: PRODUCTS, storageId: HAIR, categoryId: 'bath',
    })
    expect(merged.get('p-repair').balanceBase).toBe(60)

    const wide = catalogueBalanceRows({
      balances: BALANCES, products: PRODUCTS, storageId: null, categoryId: 'bath',
    })
    expect(wide.get('p-repair').balanceBase).toBe(100)
  })

  it('takes one options object, so a folder has nowhere to arrive positionally', () => {
    // The shape half. A second positional parameter is how a folder sneaks in
    // without any call site reading as though it did.
    expect(catalogueBalanceRows.length).toBe(1)
  ...
  it('keeps the number even when the tree has filtered that folder away', () => {
    ...
    const visibleFolders = foldersForStorage(CATEGORIES, HAIR, LINKS)
...
describe('the rows follow the folder, and the folder alone', () => {
  it('narrows to the chosen folder rather than to the storage', () => {
    ...
    const rows = catalogueRows({
      products: PRODUCTS,
      categories: foldersForStorage(CATEGORIES, HAIR, LINKS),
      categoryId: 'bath',
    })
