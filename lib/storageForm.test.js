...
describe('validateStorage', () => {
  ...
  it('refuses ticked with both boxes empty — the row cannot hold it', () => {
    // 🔴 A FOURTH STATE THE CHECKBOX ITSELF CREATED. The box exists so "no
    // policy" is an explicit choice instead of an implicit blank; ticked-and-
    // empty is a new implicit blank pointing the other way.
    //
    // ⚠️ AND ITS FAILURE WAS SILENT, WHICH IS WORSE THAN A REFUSAL. Traced
    // before this refusal existed: validate passed, the payload wrote
    // null/null, and reopening derived the tick from the row and showed it
    // UNTICKED. Ticked, saved, came back undone — no message, no trace.
    expect(validateStorage({ ...common, fineEnabled: true, finePercent: '', fineBasis: '' }))
      .toBe('products:storageDialog.fineEnabledButEmptyError')
    expect(validateStorage({ ...common, fineEnabled: true, finePercent: '  ', fineBasis: '' }))
      .toBe('products:storageDialog.fineEnabledButEmptyError')
  })

  it('an absent fineEnabled still reads two blanks as "no policy"', () => {
    // 🔴 THE STANDING TEST ABOVE CAUGHT THIS. Written as `finePolicyOn && …`
    // the refusal fired for `undefined` too, and a caller from before the box
    // says "no policy" exactly this way — the save the rule exists to allow.
    // The fourth state is only reachable by ticking, so only `=== true` refuses.
    expect(validateStorage({ ...common, finePercent: '', fineBasis: '' })).toBe('')
    expect(storagePayload({ ...common, finePercent: '', fineBasis: '' }))
      .toMatchObject({ fine_percent: null, fine_basis: null })
  })

  it('the same two empty boxes are fine when the checkbox is off', () => {
    // The mirror of the case above, and the reason the refusal is conditional:
    // unticked-and-empty is exactly how a storage says it has no policy.
    expect(validateStorage({ ...common, fineEnabled: false, finePercent: '', fineBasis: '' }))
      .toBe('')
  })
  ...
  // 🔴 A DELIBERATE PROXY, AND THE THREE THINGS A PROXY HAS TO CARRY OR IT GETS
  // DELETED IN SIX MONTHS AS "that odd test that reads source".
  //
  //   WHAT IT STANDS IN FOR — that skipping the fine for an unticked storage
  //     does not skip anything else in validateStorage.
  //   WHY NOT BEHAVIOURAL TODAY — there is no check AFTER the fine block, so no
  //     input can tell a guarded block from an early return. Measured: the
  //     early return was put back deliberately and all 33 tests stayed green.
  //     The test that looked like proof asserted `kind` and `name`, and both
  //     are checked at the TOP, before the skip.
  //   WHEN IT IS REPLACED — the first check appended after the fine block makes
  //     the claim testable by input. Assert that instead and delete this.
  //
  // ⚠️ And it is brittle on purpose-adjacent things: a blank line before the
  // closing brace breaks the endsWith. That is acceptable in a proxy and is
  // said here so a strange red is read as "the shape moved", not as a bug.
  it('the fine skip is a guarded block, not an early return', () => {
    const src = readFileSync(join(__dirname, 'storageForm.js'), 'utf8')
    const from = src.indexOf('export function validateStorage')
    // ⚠️ Cut at the function's own closing brace, not at the next export. The
    // gap between them holds prose, and prose quoting `return ''` is not code:
    // the first version counted the very sentence that explains this test.
    const body = src.slice(from, src.indexOf('\n}', from) + 2)
    // ⚠️ Both comment forms, because dropping only `//` leaves a future block
    // comment able to redden this test with an innocent quotation.
    const code = body
      .replace(/\/\*[\s\S]*?\*\//g, '')
