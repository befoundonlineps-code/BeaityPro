 ...
 */
// 🔴 **«فارغ» ليس «صفرًا»، و`Number(null)` صفرٌ منتهٍ.** الشرطُ القديم
// `Number.isFinite(Number(x))` لا يردّ `null` ولا `undefined` ولا `''` — وكلُّها
// تصير صفرًا يمرّ منه.
//
// ⚠️ **ولم يظهر ذلك ما دام الاتّجاهُ صعوديًّا وحدَه:** `counted <= recorded`
// يُسقط الصفرَ قبل أن يبلغ أيَّ حكم، **فنجا الشرطُ بالمصادفة لا بالحراسة.**
// **وأوّلُ ما بُني الاتّجاهُ الهابط ومض على `null` مقابلَ ١٥٠** — عدٌّ لم
// يُكتب، قُرئ رفًّا أُفرِغ.
//
// 🔴 **وهو الجذرُ نفسُه للمرّة الخامسة في هذا الموديول** — بعد «الفشل ليس
// فراغًا» و«بلا سعر ليس سعره صفر» و«لا أعرف ليست مجّانًا» و«صفرٌ مكتوبٌ ليس
// خانةً لم تُلمس». **ويُغلق هنا لكلتا الدالّتين، لا للجديدة وحدَها.**
...
export function jumpsAboveRecord(countedBase, recordedBase) {
  const counted = countOrNull(countedBase)
  const recorded = countOrNull(recordedBase)
  if (counted === null || recorded === null) return false
...
/**
 * وهل يهبط هذا العددُ تحت المسجَّل هبوطًا يستحقّ سؤالًا؟
 *
 * 🔴 **قرارُ المالك (٢٤ آب ٢٠٢٦): ٣.١٣ب صار ثنائيَّ الاتّجاه.** والسطرُ الذي
 * كان يقول «الاتّجاهُ صعوديٌّ بقرار المالك» فوق `jumpsAboveRecord` صار تاريخًا،
 * **ويبقى هناك لأنّ ذاك الشرطَ ما زال يفعل ذلك — والاتّجاهُ الثاني هنا.**
 *
 * ⚠️ **وحادثتُه: تُكتب `5` والمقصودُ `50`** — رقمٌ واحدٌ ناقصُ خانة، في أوّل
 * جردٍ لهذا المستودع ⇒ **عجزٌ وهميٌّ ٤٥، والترحيلُ يكتب غرامةً عند العجز.**
 * ولا يمسكه شيءٌ قبل هذا: `counted_base >= 0` وحدَه، و`count_invalid` عند
 * السالب وحدَه، **و«قطعٌ صحيحةٌ فقط» يحكم على الكسر لا على المقدار — و`5`
 * عددٌ صحيح.**
 *
 * 🔴 **والكلفةُ مُعلَنةٌ لا مسكوتٌ عنها، وهي ما جعلت القرارَ ممكنًا:** سيومض
 * **أيضًا** في العجز الحقيقيّ الكبير — **وهو الحالةُ التي وُجد هذا الموديولُ
 * لأجلها.** ⇒ **فقُبل التنبيهُ على الحالة المشروعة ثمنًا لرؤية الخطأ، ولم
 * يُطلب حارسٌ يفرّق بينهما.** ⚠️ **ومنه الصفر: «عدّينا فما لقينا» عدٌّ مشروعٌ
 * وسيومض** — **وذلك داخلٌ في الثمن المقبول، لا سهوٌ في الشرط.**
 *
 * ⚠️ **والحافّتان هما نفسُهما، مقلوبتَين لا مضاعَفتَين:** نسبةٌ عشرةٌ وفرقٌ
 * عشرون. **ورقمان ثانيان للهبوط يجعلان جدولَ المقابلة جدولين، ولا سندَ
 * لتفريقهما** — فما لم يُقَس فرقٌ، لا يُخترع له رقم.
 *
 * 🔴 **والمرآةُ ليست حجّةً بذاتها، والاتّجاهان غيرُ متماثلَين في طبعهما:**
 * القفزةُ فوق المسجَّل **خطأُ إدخالٍ غالبًا** (رقمُ عبوةٍ مكانَ رقمِ حبّة)،
 * **والهبوطُ قد يكون واقعًا** (رفٌّ أُفرغ فعلًا) ⇒ **فمعدَّلُ الإنذار الكاذب
 * هنا أعلى بطبعه.** ⚠️ **ورقمٌ يخفّفه قرارٌ لا اشتقاق، ولا قياسَ يسنده اليوم**
 * — **فبقيت الحافّتان كما هما، والكلفةُ مقبولةٌ بنصّ قرار المالك.**
 * ⇒ **ومن يعدّلهما للهبوط وحدَه يفصل الجدولَ جدولين، فليكتب سببَه هنا.**
 *
 * ⚠️ **والصفرُ المكتوبُ صراحةً لا يومض دائمًا — والأرضيّةُ تحرسه:** مقيسًا،
 * `0` مقابلَ مسجَّلٍ `5` أو `15` أو `19` **صامت**، ومقابلَ `20` فأكثرَ **يومض.**
 * ⇒ **فرفٌّ صغيرٌ أُفرِغ لا يُزعج، ورفٌّ كبيرٌ يُسأل عنه** — وذلك أثرُ
 * `ANOMALY_FLOOR` لا شرطٌ ثانٍ.
 */
export function dropsBelowRecord(countedBase, recordedBase) {
  const counted = countOrNull(countedBase)
  const recorded = countOrNull(recordedBase)
  if (counted === null || recorded === null) return false
  if (recorded <= 0) return false
  if (counted >= recorded) return false
  // ⚠️ `counted * RATIO <= recorded` لا `recorded / RATIO >= counted` — القسمةُ
  // تُدخل كسرًا عائمًا في حكمٍ حدُّه بالضبط هو ما يُقاس، والضربُ لا يفعل.
  if (counted * ANOMALY_RATIO > recorded) return false
  return recorded - counted >= ANOMALY_FLOOR
}
 ...
 *
 * 🔴 **والاتّجاهان معًا في `FLAGGED` واحدة، لا حالةٌ ثالثة.** والحجّةُ ليست
 * صغرَ التعديل: **الهابطُ هو الذي يكتب الغرامة** (`post_stocktake_session`
 * تُغرِّم على العجز لا على الفائض) ⇒ **فلوحٌ يجمع الشاذَّ ويُسقط الهبوطَ
 * يُسقط الصنفَ الوحيدَ الذي ينتهي بخصمٍ من راتب.**
 *
 * ⚠️ **وحالةٌ واحدةٌ للاثنين تعني أنّ الاتّجاهَ لا يُقرأ من الحالة** ⇒ **فيجب
 * أن يُقرأ من الصفّ** — وهو كذلك: الصفُّ يطبع **المعدودَ والمسجَّلَ معًا**
 * (`StocktakingSheet.js:312-338`)، **فالقارئُ يرى أيَّهما أكبر بلا وسم.**
 * **والفعلُ المطلوبُ يختلف بالاتّجاه** — إعادةُ عدٍّ في الصاعد، وبحثٌ عن فقدٍ
 * في الهابط — **وهما في `jumpHintHelp` و`dropHintHelp` على الورقة نفسِها.**
 *
 * ⚠️ **والنفيُ يُقال صراحةً: هبوطٌ دون الأرضيّة لا يدخل اللوح.** `ANOMALY_FLOOR`
 * تحكم الاتّجاهين معًا، **فمنتجٌ مسجَّلُه ٥ لا يُومض ولا يُجمع مهما هبط.**
 *
 * 🔴 **وهذا قرارٌ مغلَقٌ لا حالةٌ مؤقّتة (٢٦ آب ٢٠٢٦)، بلفظ المالك:** «لا،
 * بنخلّي الوضع زي ما هي — بيحذّر بس إذا الفرق ٢٠ فأكتر». ⇒ **فلا يُقرأ
 * السطرُ نقصًا ينتظر رقمًا، ولا يُفتح البابُ من جديد بلا واقعةٍ جديدة.**
 *
 * ⚠️ **وكيف وصل القرارُ جزءٌ منه:** سُئل أوّلًا بحالةٍ واحدةٍ تبرّر البناء
 * (١٥ ⟶ صفر) فقال «ممكن يحذّر عادي» — **ثمّ عُرضت الحالةُ التي تكشف الكلفة**
 * (٣ ⟶ صفر) **فقال «خلّيها زي ما هي».** ⇒ **والإذنُ الأوّلُ كان صريحًا وكان
 * سيُنتج قاعدةً لا يريدها** — **فالناقصُ في السؤال لا في الجواب.**
 *
 * ⇒ **ولو دخل اللوحَ وحدَه لظهر سطرٌ في اللوح لا يومض في الورقة**، وذلك
 * تناقضٌ بين شاشتين.
 */
export function linesToConfirm({ counts, uoms, rows, products } = {}) {
  ...
  for (const productId of Object.keys(counts || {})) {
    ...
    if (countedBase === null || !row) {
      ...
      jumpsAboveRecord(countedBase, row.plan)
      || dropsBelowRecord(countedBase, row.plan)
    ) {
