...
describe('nothing hand-rolls an isolate or a range beside it', () => {
  it('keeps dir="ltr" to the three places that own a direction decision', () => {
    // TimeRange owns clock ranges. WorkPhoneDialog owns phone numbers, which
    // are the same problem in a different shape.
    //
    // 🔴 LtrNumber owns the third: a number that may carry a SIGN. Measured
    // from a real engine, not reasoned — the stocktaking sheet's first
    // screenshot drew `-2` as `2-` and `-3` as `3-`, because the minus is a
    // neutral and takes the paragraph's direction.
    //
    // ⚠️ AND THIS GUARD IS WHY THERE ARE THREE AND NOT FOUR: the fix was first
    // written as a local `<span dir="ltr">` inside StocktakingSheet.js — the
    // seventh hand-rolled isolate this file exists to stop — and it failed
    // here by name. The owner was extracted rather than the list widened.
    const withIsolate = ALL.filter((f) => read(f).includes('dir="ltr"'))
    expect(withIsolate.sort()).toEqual([
      'components/LtrNumber.js',
  ...
  it('finds no dash-joined pair that has not been looked at', () => {
    ...
    const REVIEWED = [
      ...
      // ⚠️ A SECOND COPY OF THAT LINE LIVED HERE FOR ONE ROUND, in a
      // reference-styled tab strip built to replace Sidebar.js. The bars turned
      // out not to be part of the conversion at all, the strip was deleted, and
      // the entry went with it — which is the exception list doing its job in
      // the direction nobody watches: it failed when the file APPEARED and
      // failed again when it VANISHED, so neither could pass unnoticed.
      ...
      // an employee's name and their role: two Arabic phrases, and the
      // translated role is strong RTL text, so the dash takes the paragraph
      // direction it already wanted. Neither side is a number.
      "components/StorageFormDialog.js :: {e.name} — {t(`employees:roles.${e.role}`)}",
