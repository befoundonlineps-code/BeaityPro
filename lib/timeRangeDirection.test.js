...
describe('nothing hand-rolls an isolate or a range beside it', () => {
  ...
  it('finds no dash-joined pair that has not been looked at', () => {
    ...
    const REVIEWED = [
      ...
      // The same line, in the reference-styled tab strip that will replace
      // Sidebar.js when the products screen is approved. Two Arabic phrases,
      // neither of them a number — and when Sidebar goes, this entry stays and
      // that one goes with it.
      "components/ref/RefTopBar.js :: title={s.active ? label : `${label} — ${t('common:inDevelopmentSuffix')}`}",
