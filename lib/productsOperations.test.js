...
describe('an operation is refused by the same rule the bar used', () => {
  it('blocks the eight that need one real storage while the lens is wide', () => {
    // ⚠️ `orders` JOINED THIS LIST and used to be in the one below. The order
    // screen now opens on «which of THIS storage's folders», so «all storages»
    // has nothing to draw. The row it writes is unchanged — see
    // lib/storageScope.test.js for why the two questions differ.
    for (const op of ['supply', 'write_off', 'return_to_supplier', 'transfer', 'stocktake', 'coverage', 'balances', 'orders']) {
  ...
  it('lets the rest through from anywhere', () => {
    for (const op of ['documents', 'suppliers', 'storages']) {
...
describe('what the reference offers and we do not', () => {
  it('names the deleted entries rather than drawing them empty', () => {
    ...
    // an omission somebody notices in a screenshot. Two buttons: a product price
    // sheet we do not have, and fixed assets we have no table for.
    expect(REFERENCE_ENTRIES_WITHOUT_DATA).toEqual([
      'setPricesForProducts', 'fixedAssets',
    ])
  })

  // 🔴 THE CARET LEFT THIS LIST, AND THAT DEPARTURE IS PINNED — not because the
  // name is gone, but because it may only be gone while the menu it describes
  // is actually drawn. Deleting the entry and never building the menu would put
  // the caret back to revealing one live route with nothing beside it, which is
  // exactly what the entry existed to prevent.
  it('🔴 the stocktake caret is out of the list ONLY because the menu draws all three', () => {
    expect(`مذكورٌ في القائمة: ${REFERENCE_ENTRIES_WITHOUT_DATA.includes('stocktakeInputMethods')}`)
      .toBe('مذكورٌ في القائمة: false')

    const menu = fs.readFileSync(path.join(__dirname, '..', 'components/StocktakeMethodMenu.js'), 'utf8')
    // ⚠️ Derived from the file, not a count written here: every method the menu
    // names, live or disabled. Three is the reference's number, and a fourth
    // route built tomorrow raises it without a line changing.
    const live = [...menu.matchAll(/data-method="(\w+)"/g)].map((m) => m[1])
    const inert = [...menu.matchAll(/id="(\w+)"/g)].map((m) => m[1])
    expect(`حيّ: ${live.join(' · ') || 'لا شيء'} · معطَّل: ${inert.join(' · ') || 'لا شيء'}`)
      .toBe('حيّ: manual · معطَّل: barcode · excel')
  })
})
