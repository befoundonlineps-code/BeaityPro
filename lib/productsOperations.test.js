...
import {
  OPERATIONS, ALL_OPERATIONS, OPERATION_LABEL_KEY,
...
describe('the operation set is the view set, minus the screen itself', () => {
  ...
  it('is one list, not two', () => {
    // 🔴 It was briefly split — a toolbar list and a «storage box» list — to
    // follow where the reference launches each one from. That band is not being
    // copied any more, so the split lost its subject. And a split is where an
    // operation goes missing: one that fell out of BOTH halves would be
    // unreachable while every other test kept passing, which is the exact shape
    // of «opening and prices exist and no button renders them» that this module
    // has already had once.
    expect(ALL_OPERATIONS).toBe(OPERATIONS)
    expect(new Set(OPERATIONS).size).toBe(OPERATIONS.length)
  })

  it('still reaches the storages editor from the bar', () => {
    // It had been moved out to sit beside the storage picker, as the reference
    // does. The bar is ours again, so it comes back into it.
    expect(OPERATIONS).toContain('storages')
  })
})

describe('the bar draws exactly this set, in its own order', () => {
  const bar = fs.readFileSync(
    path.join(__dirname, '..', 'components', 'ProductsSecondaryBar.js'), 'utf8'
  )
  const drawn = [...bar.matchAll(/\{\s*view:\s*'([a-z_]+)'/g)].map((m) => m[1])

  it('draws every operation exactly once', () => {
    ...
    expect(drawn.slice().sort()).toEqual([...OPERATIONS].sort())
  ...
  it('keeps the order the bar argues for, not the reference band’s', () => {
    ...
    // documents.
    expect(drawn.slice(0, 4)).toEqual(['storages', 'suppliers', 'orders', 'supply'])
  })
})
