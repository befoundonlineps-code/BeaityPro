...
// جدولُ المجلّدات وأبنائها، وحسابُ كلِّ عمود — **يخدم أربعَ شاشاتٍ لا واحدة.**
//
// 🔴 **وكانت هذه الترويسةُ تقول «جدولُ ‹طلب بضاعة›»** — صحّت يومَ كُتبت، ثمّ
// صار الملفُّ مقروءًا من: `OrderProductsScreen` · `SupplyProductsScreen` ·
// `OrderDocumentView` · `SupplyDocumentView`، **ومن `returnGrid` و
// `writeOffGrid` و`writeOffFromInvoice` بدائيّاتٍ.** فاسمُ شاشةٍ واحدةٍ في
// الترويسة **يُقرأ حدًّا للملفّ وهو ليس حدَّه** — وهو الصنفُ المحذَّر منه في
// `CLAUDE.md`: «ترويسةٌ صحّت يومَ كُتبت وصارت تُقرأ ادّعاءً أوسعَ منها».
// ⚠️ **وكشفه المراجعُ من المواصفة**: «التوريدُ وحدَه مرجعُه ملفٌّ باسم شاشةٍ
// أخرى — هل هو مشترك، أم خطأُ نسخ؟»
//
...
export function perPackage(product) {
...
// yet».
export function amountOf(product, rawPackages, rawPrice) {
  const packages = packagesOf(rawPackages)
  if (packages === null) return null
  const price = effectivePrice(product, rawPrice)
  if (price === null) return null
  return roundToPlaces(packages * price)
}

// 🔴 سعرُ السطر السارِي: **ما كُتب في الخانة إن وُجدت، وإلّا سعرُ الكتالوج.**
//
// ⚠️ **والمبلغُ يُحسب من هذا وحدَه.** شاشةُ التوريد صار فيها عمودُ تكلفةٍ قابلٌ
// للتعديل (قرارُ المالك)، **وحسابُ المبلغ من الكتالوج بينما الخانةُ تعرض رقمًا
// آخرَ هو رقمان لسؤالٍ واحد** — وهي العلّةُ التي يلاحقها هذا المشروعُ منذ
// `hasKnownValue`. فمصدرٌ واحدٌ للسعر، هنا.
//
// ⚠️ **والفراغُ فراغٌ لا صفر.** منتجٌ بلا سعرِ شراءٍ في الكتالوج يعطي خانةً
// فارغةً ومبلغًا `null` — **ولا يعطي صفرًا**. وهذا ما يجعل شاشةَ التوريد ترفضه
// قبل أن يراه `Number()`، لأن `unit_cost` مختومٌ على كلّ صرفٍ بعده و«لا أعرف»
// المكتوبةَ صفرًا لا تُستدرَك.
export function effectivePrice(product, rawPrice) {
  const typed = String(rawPrice ?? '').trim()
  const source = typed !== '' ? typed : product?.[ORDER_PRICE_COLUMN]
...
// ما تعرضه خانةُ السعر: المكتوبُ إن كُتب، وإلّا سعرُ الكتالوج نصًّا.
//
// ⚠️ **ولا يُحوَّل العدمُ إلى `'0'`** — خانةٌ فارغةٌ تقول «ما في سعرٌ مسجَّل»،
// وصفرٌ يقول «مجّانًا»، والفرقُ بينهما هو الفرقُ الذي بُني عليه الموديول.
export function priceBoxValue(product, rawPrice) {
  const typed = String(rawPrice ?? '').trim()
  if (typed !== '') return typed
  const stored = product?.[ORDER_PRICE_COLUMN]
...
// ⚠️ **مُصدَّرةٌ كي تقرأها شاشةُ العرض بنفس الدالّة** — «الرصيد الحاليّ» يُعرض
// في العرض كما في الإنشاء (قرارُ المالك)، **ونسخةٌ ثانيةٌ منه هي فرصةٌ ثانيةٌ
// لتفترق عن رقم شاشة الإدخال.**
export function balanceIndex(balances, storageId) {
  const index = new Map()
...
export function orderGridRows({
  selectedFolderIds, categories, products, balances, storageId, packages, prices,
} = {}) {
  ...
  // ⚠️ اختياريٌّ عمدًا: شاشةُ الطلب بلا عمود سعرٍ إطلاقًا، فتمرّ بلا هذا
  // ويأتي السعرُ من الكتالوج كما كان. **وشاشةٌ واحدةٌ تكسب العمودَ لا تفرضه
  // على الأخرى.**
  const typedPrices = prices || {}
  const rows = []
  ...
  for (const folder of folderTickRows(categories)) {
    ...
    const children = own.map((product) => ({
      ...
      price: priceBoxValue(product, typedPrices[product.id]),
      amount: amountOf(product, typed[product.id], typedPrices[product.id]),
    }))
