...
//
// ⚠️ AND THE DECISIVE REASON IS NOT COST, it is what a function would have to
// duplicate. SECURITY DEFINER bypasses RLS, so the eight policies measured and
// verified in 053a/053b would stop being the gate — the function would derive
// salon_id itself, and an order has no storage to derive it from the way
// post_stock_document does. Its only source is auth.uid() -> profiles: the
// policy predicate, rewritten in PL/pgSQL. That buys atomicity with a SECOND
// STATEMENT OF THE ISOLATION RULE, on the one thing whose silent divergence
// means one salon reading another. ADR-055.
//
// ⚠️ Revisit when the order gains anything a NUMBER depends on — a state, a
// received quantity, any reconciliation against the supply. What makes the
// direct write acceptable today is not that it is simpler; it is that its half
// -written state lies to nobody.
//
