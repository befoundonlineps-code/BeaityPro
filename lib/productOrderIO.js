...
//
// ❌ A THIRD REASON WAS WRITTEN HERE AND IS WRONG. It said a function would
// bypass RLS through SECURITY DEFINER and have to restate the isolation rule in
// PL/pgSQL. None of the four stock functions is SECURITY DEFINER — 051b:28 says
// so explicitly, and 050c measured prosecdef = false — so they run as the
// invoker and RLS applies inside them. A function here would duplicate nothing.
//
// ⚠️ Left as a correction rather than deleted, because the reviewer endorsed
// that argument by name. The failure is the one CLAUDE.md 4b had just been
// written about: "SECURITY DEFINER bypasses RLS" is true on its own, and I
// assumed these functions were, without reading them.
//
// ⚠️ So the decision stands on the two reasons above alone, and it drops from
// COMPELLED to JUSTIFIED. Which makes the trigger below heavier, not lighter:
// revisit when the order gains anything a NUMBER depends on — a state, a
// received quantity, any reconciliation against the supply. What makes the
// direct write acceptable today is not that it is simpler; it is that its
// half-written state lies to nobody. ADR-055.
//
