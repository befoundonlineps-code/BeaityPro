...
const FRACTIONAL_ALLOWED = [
  {
    file: 'ProductFormDialog.js', step: '0.001', count: 4,
    why: 'كمّياتُ التعبئة والتجزئة: «بالعبوة» و«ناتج التجزئة» و«حجم الجزء» ومكوّناتُ الطقم. منتجٌ بالمل أو الغرام كمّيّتُه كسريّةٌ بطبعها، ولم يطلب المالكُ تغييرَها.',
  },
  {
    file: 'ProductOrderScreen.js', step: '0.001', count: 1,
    why: 'كمّيّةُ سطر الطلبيّة — تُطلب بنفس وحدة المنتج، فمنتجُ المل يُطلب بكسر.',
  },
  {
    file: 'StockDocumentScreen.js', step: '0.001', count: 2,
    why: 'كمّيّةُ السطر والبونص في المستند — كسريّةٌ لنفس سبب حقول المنتج.',
  },
]
...
describe('the arrows on a money field move by a whole unit', () => {
  ...
  it('leaves no money field on a fractional step', () => {
    // 🔴 `0.01` كان على كلِّ حقلِ سعرٍ في النظام — وهو ما جعل السهمَ عديمَ
    // الفائدة. ولا استثناءَ له: كلُّ ما كان بـ`0.01` مالٌ أو نسبة، والاثنان
    // يتحرّكان بواحد.
    const offenders = []
    for (const file of files) {
      const text = fs.readFileSync(file, 'utf8')
      if (text.includes('step="0.01"')) offenders.push(path.basename(file))
    }
    expect(offenders).toEqual([])
  })

  it('keeps the reorder threshold on a whole step — the field that showed 30.002', () => {
    const dialog = fs.readFileSync(path.join(ROOT, 'components', 'ProductFormDialog.js'), 'utf8')
    const at = dialog.indexOf('value={lowSupplyUnits}')
    expect(at).toBeGreaterThan(-1)
    // السطرُ نفسُه، لا الملفُّ كلُّه: الملفُّ فيه حقولٌ كسريّةٌ مشروعة.
    const line = dialog.slice(dialog.lastIndexOf('<Input', at), at)
  ...
  it('accounts for every fractional step that remains, with a reason', () => {
    ...
    for (const file of files) {
      const text = fs.readFileSync(file, 'utf8')
      for (const step of ['0.001', '0.005', '0.1', '0.5']) {
