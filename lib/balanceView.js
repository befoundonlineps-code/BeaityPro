...
// paid — item 34's `cost_is_estimated`, raised to the balance by bool_or.
//
// ⚠️ Sticky forever, and deliberately. A stamped cost is never recomputed
// (ADR-051), so a balance whose history contains one estimate has an average
// that is partly estimated for good — even after ten later deliveries at real
// prices, because the average is derived from ALL the movements. The average
// does not forget.
//
// That is sticky, and it is the honest answer: it is what makes "how does it
// go away?" true — reversing the document is genuinely the only exit, the same
// as ADR-051 and the same as what the owner did by hand with the poisoned pair.
//
// ⚠️ And it reads FALSE until the column exists. The script is prepared and not
// run, so `cost_has_estimate` is undefined on every row today; the badge stays
// dark and lights up on its own the moment the view carries it. Nothing here
// invents a value for a column that is not there.
