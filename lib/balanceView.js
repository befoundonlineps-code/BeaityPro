...
// and not `balances`.
// 🔴 THE LOW-SUPPLY THRESHOLD IS COMPARED AGAINST THE PRODUCT'S TOTAL ACROSS
// EVERY STORAGE — the owner's decision, taken after the grain was measured.
//
// low_supply_units is a column on PRODUCTS: one number per product. No
// per-(product, storage) threshold exists anywhere in the schema — 080_1b
// listed every column of all three tables. So comparing it against ONE
// storage's balance was the claim the data cannot carry: the same n judging a
// cabin that holds a working stock and the main storage behind it.
//
// ⇒ «Is the salon running low on this» is the question the number can answer,
// and it is what a reorder point is for.
//
// ⚠️ ONE COMPUTATION, NOT TWO. The balances screen, the stocktake sheet and the
// catalogue column all read it from here, so no second copy can drift. Same
// reason this module is shared at all, applied to the number rather than to the
// rows.
function salonWideTotals(balances) {
  const totals = new Map()
  for (const row of balances || []) {
    totals.set(row.product_id, (totals.get(row.product_id) || 0) + Number(row.balance_base))
  }
  return totals
}
...
export function balanceRows({ balances, products, storageId }) {
  ...
  // ⚠️ Off the UNFILTERED rows on purpose: the caller narrows to one storage,
  // and the threshold does not.
  const acrossStorages = salonWideTotals(balances)
  ...
  return (products || []).flatMap((product) => {
    ...
    return [{
      ...
      // ⚠️ The SALON's total, not this storage's — see salonWideTotals above.
      // A cabin down to its last two while the main storage holds two hundred
      // is not a salon running low, and it used to light up as one.
      lowSupply: threshold !== null && (acrossStorages.get(product.id) ?? 0) <= threshold,
    }]
  })
}
