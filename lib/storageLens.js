...
// The value that means "the whole salon". A sentinel rather than '' or null,
// because those already mean "nothing chosen yet" — and the two must not
// collapse: nothing-chosen falls back to a real storage, all-storages does not.
export const ALL_STORAGES = 'all'

// ⚠️ FAILS CLOSED. A view this does not know about cannot widen, so a caller
// that forgets to pass one gets a real storage. The asymmetry is deliberate:
// showing a catalogue for one storage when the person meant all of them is a
// narrower view than they asked for, and posting a stocktake against "all" is
// not a view at all.
