...
export function currentLens(storages, chosen, view) {
  ...
  // 🔴 NOBODY HAS CHOSEN — AND ON A VIEW THAT CAN MEAN THE WHOLE SALON, THE
  // WHOLE SALON IS THE ANSWER.
  //
  // ⚠️ This line used to fall straight to defaultLens, so the catalogue opened
  // on the first live storage after a hard refresh. That is the implicit choice
  // this module spends its rounds removing, arriving through the fix for
  // another one: the catalogue's own picker had opened on ALL, and when it was
  // merged away the default came here and was not brought with it.
  //
  // ⚠️ AND A TEST PINNED THE BUG AS CORRECT — «still tells ALL apart from
  // nothing-chosen» asserted that '' on the catalogue gives the first storage.
  // Written from what the code did rather than from what it should do, which is
  // the same fault as the duplicate storage_not_empty entry whose test agreed
  // with the accident.
  //
  // ⚠️ It changes the DOCUMENT LIST's default too, deliberately: it also used to
  // open on the first storage, through its own unchecked "all" box. Showing one
  // storage's documents because nobody has said otherwise is the same silent
  // pick. Both views can answer for the salon, so before anybody chooses, both
  // do.
  return lensMayWiden(view) ? ALL_STORAGES : defaultLens(storages)
}
