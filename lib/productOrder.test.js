import {
  orderLine, orderPayload, orderRowsFromLines, orderTotal, documentRowsHoldWork, mergeFilledRows,
} from './productOrder'
...
// ==========================================================================
// ⚠️ THE ONE OPERATION IN THIS FEATURE THAT CAN DESTROY TYPED WORK, and it used
// to live inside a setState callback where no test could reach it. Extracted
// for exactly that reason: the failure it guards against is somebody losing
// three lines they cannot prove they wrote.
// ==========================================================================
describe('pouring an order into rows that already have some', () => {
  const typed = { ...emptyDocumentRow(), productId: 'p1', enteredQuantity: '9' }
  const blank = emptyDocumentRow()
  const poured = [{ ...emptyDocumentRow(), productId: 'p2', enteredQuantity: '1' }]

  it('replacing keeps nothing that was there', () => {
    expect(mergeFilledRows([typed, blank], poured, true)).toEqual(poured)
  })

  it('appending keeps the typed rows and puts the order under them', () => {
    const merged = mergeFilledRows([typed, blank], poured, false)
    expect(merged.map((r) => r.productId)).toEqual(['p1', 'p2'])
  })

  it('appending drops the untouched blanks rather than keeping them', () => {
    // ⚠️ Not tidying. A blank row sitting between the kept lines and the poured
    // ones reads as a line somebody started, and the row after a blank is where
    // people stop looking.
    expect(mergeFilledRows([blank, typed, blank], poured, false)).toHaveLength(2)
  })

  it('survives being handed nothing at either end', () => {
    expect(mergeFilledRows(null, poured, false)).toEqual(poured)
    expect(mergeFilledRows([typed], null, false)).toEqual([typed])
  })
})
