...
// in six months.
//
// Direction matters, and only one direction is asserted: every code RAISED
// must be NAMED. Not the reverse — six named codes have no source in docs/
// (both trigger codes, and transfer_same_storage from transfer_stock), and
// that asymmetry is a fact about the snapshot being partial, not a fault in
// the map. Asserting the reverse would delete measured knowledge to satisfy an
// incomplete file.
describe('every raise written down in docs/ has a named sentence', () => {
  ...
  it('covers post_stock_document and post_stocktake exhaustively', () => {
    // Both texts arrived COMPLETE from pg_get_functiondef, so this is a
    // stronger claim than the rest of the map makes: nine raises across the
    // two functions, seven distinct codes, all named. It is asserted against
    // the pasted bodies rather than from memory of reading them.
