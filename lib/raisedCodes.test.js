...
describe('every raise written down in docs/ has a named sentence', () => {
  ...
  // ⚠️ A CODE THAT THE SAME FILE CATCHES NEVER REACHES A HUMAN, so demanding a
  // translated sentence for it is asking for a sentence nobody can ever read.
  //
  // The case is real and arrived with 095b: a verification file writes test
  // rows, then `raise exception 'ROLLBACK_MARKER'` to undo them, and catches
  // that exact string two lines later. It is a control-flow token, not a
  // refusal — CLAUDE.md item 1ب is written about this exact pattern.
  //
  // 🔴 AND THE EXEMPTION IS DERIVED, NOT A NAME LIST. A list would fail open:
  // the next probe invents `UNDO_MARKER` and passes unexamined. What is checked
  // instead is the PROOF that it is caught — the file comparing sqlerrm against
  // that same literal. A marker raised with no matching catch is still flagged,
  // which is the direction that matters: it would escape to a real caller.
  //
  // ⚠️ Per file, deliberately. A code caught in one file and raised bare in
  // another still needs its sentence, because the second file is the one that
  // reaches somebody.
  const caughtIn = (text) =>
    new Set([...text.matchAll(/sqlerrm\s*(?:<>|=|!=)\s*'([a-z_]+)'/gi)].map((m) => m[1]))
  ...
  for (const file of files) {
    ...
    const caught = caughtIn(text)
    for (const code of codesIn(text)) {
      if (caught.has(code)) continue
  ...
  it('exempts a caught marker and still catches an uncaught one', () => {
    // Counter-evidence for the exemption itself, on strings — nothing touches
    // the work tree. The first is the 095b probe shape; the second is the same
    // marker with its catch removed, which MUST still be demanded, because a
    // raise nobody catches travels to whoever called the function.
    const withCatch = "raise exception 'ROLLBACK_MARKER';\nexception when others then\n"
      + "  if sqlerrm <> 'ROLLBACK_MARKER' then null; end if;"
    const without = "raise exception 'ROLLBACK_MARKER';"
    expect(caughtIn(withCatch).has('ROLLBACK_MARKER')).toBe(true)
    expect(caughtIn(without).has('ROLLBACK_MARKER')).toBe(false)
    // And a genuine refusal is never exempted: nothing compares sqlerrm to it.
    expect(caughtIn("raise exception 'unit_cost_required' using hint = 'x';").size).toBe(0)
  })
