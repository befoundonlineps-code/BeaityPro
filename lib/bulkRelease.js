...
// The two dates picked in the dialog turned into the half-open window both
// the preview and the function work in.
//
// `to` is the start of the day *after* the last one chosen, so a single-day
// absence covers that whole day and the comparison stays `< to` everywhere —
// the same half-open rule the resource and employee constraints already use,
// rather than a second convention that only this feature obeys.
//
// Returns null for a range that runs backwards, so the caller reports it
// instead of quietly releasing nothing.
export function releaseWindow(fromDateISO, toDateISO) {
