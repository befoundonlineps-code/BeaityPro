...
export const RAISED_CODES = {
  ...
  // Stock. ⚠️ The first four were MEASURED — called from outside until each
  // guard fired and the code came back. RLS hides every real storage, so the
  // very first lookup answers storage_not_found and nothing behind it could be
  // reached from outside, which is why the list stopped there for a while.
  //
  // ✅ The four below them are now READ FROM THE FUNCTION TEXT rather than
  // driven at: post_stock_document and post_stocktake both arrived complete
  // from pg_get_functiondef. Reading beats driving here — the codes behind the
  // first lookup are unreachable from outside by construction, so no amount of
  // calling would ever have surfaced them.
  //
  // ✅ And because both texts are COMPLETE rather than excerpted, the entry
  // for those two functions is exhaustive, which is a stronger claim than the
  // rest of this file makes: post_stock_document raises exactly six
  // (wrong_function_for_doc_type, stock_document_empty, storage_not_found,
  // product_not_found, stock_line_zero, unit_cost_required) and post_stocktake
  // exactly three (storage_not_found, product_not_found, count_invalid). All
  // nine are here. Nothing else in either function can reach a screen.
  //
  // ⚠️ Still unread: whatever transfer_stock and reverse_stock_document raise
  // beyond transfer_same_storage, which was measured. Their text has not
  // arrived, and for them this list stays a record of what has been SEEN,
  // never completed from the design.
