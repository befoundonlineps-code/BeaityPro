...
export const RAISED_CODES = {
  ...
  // ⚠️ ٠٩٥ — محرّكُ تكلفة الدفعة، **وكلاهما نبت من قيدٍ جديدٍ لا من ميزةٍ
  // جديدة.** وقُبض عليهما بهذا الاختبار نفسِه لحظةَ كتابة السكربت — الرابعةُ،
  // وقبل أن يُشغَّل شيء.
  //
  // ⚠️ AND THE TWO ARE NOT THE SAME KIND OF REFUSAL, which is why they do not
  // share a sentence:
  //
  //   estimated_cost_negative  ⟵ FIXABLE BY WHOEVER MEETS IT. `stock_lots`
  //     CHECKs unit_cost >= 0 and `stock_movements` never did, so a negative
  //     nominal_purchase_price that used to be stamped onto a movement now
  //     refuses a whole posting. Without a named raise the person gets a bare
  //     23514 naming neither the product nor the field. The sentence names the
  //     field and the way out, because there IS one.
  //
  //   draw_quantity_invalid    ⟵ NOT REACHABLE BY ANY INPUT. Every call site
  //     passes a quantity it has already refused as zero (`stock_line_zero`,
  //     `count_invalid`) and negated into positivity. It is an internal
  //     assertion, and its sentence says so rather than pretending the reader
  //     typed something wrong — a refusal that blames the user for a defect is
  //     worse than a generic one.
  //
  // ⚠️ **ولا يُقرأ الثاني بيّنةً على أن الحالة تقع.** هو حارسُ صنفٍ لمنادٍ
  // جديدٍ يأتي لاحقًا بلا هذا الشرط، **لا وصفٌ لشيءٍ وقع** — والفرق بين
  // «حارسٌ لحالةٍ قادمة» و«حارسٌ كشف علّة» يستاهل يُقال.
  estimated_cost_negative: 'products:stock.estimatedCostNegative',
  draw_quantity_invalid: 'products:stock.drawQuantityInvalid',

  // ⚠️ ٠٩٦ — المسارُ الصريح. **وأربعتُها يبلغها إنسانٌ فعلًا**، بخلاف
  // draw_quantity_invalid أعلاه: الشاشةُ تُرسل دفعةً وكمّيّةً كتبهما شخص، وكلُّ
  // رفضٍ هنا جوابٌ عن شيءٍ فعله.
  //
  // 🔴 STATIC EXAMPLES YES, RUNTIME VALUES NO — and the distinction is the
  // whole rule. It was first written here as "no numbers in these sentences",
  // which the sentences themselves contradict: two of them teach with 5/8 and
  // 3/12. Review caught the mismatch between the claim and the text.
  //
  // The line is not "numbers" but WHERE A NUMBER COMES FROM:
  //
  //   static example (5, 8, 3, 12)  ⟵ part of the explanation, true forever,
  //                                    and CLAUDE.md asks for an example by name
  //   runtime value ("remaining = 12") ⟵ NEVER, because the named key wins over
  //                                    the hint by design, so it would never
  //                                    reach anybody — a promise the error
  //                                    ladder structurally cannot keep
  //
  // ⚠️ Which is why lot_insufficient says "the remaining SHOWN in the batch
  // column" rather than the number: the screen is holding that value beside
  // the field the person is typing in, and the sentence points at it instead
  // of duplicating a figure it cannot supply.
  //
  // ⚠️ AND lot_insufficient IS THE ONE THAT MUST NAME A WAY OUT, because it is
  // the only one a correct person meets: the goods really did span two
  // batches. "Split into two lines" is the answer, and a sentence that only
  // said "too much" would leave them stuck at a screen that is behaving
  // correctly.
  ...
  // ⚠️ ٠٩٧ — الشطبُ صار **اختياريَّ الدفعة**، فسقط الإلزام. وهذان يحفظان
  // المبدأ الذي حلّ محلَّه: **write_off لا يفتح دفعةً مقدَّرةً أبدًا.**
  //
  // ⚠️ AND insufficient_stock IS NOT lot_insufficient. One says «this BATCH
  // has less», the other «the STORAGE has less» — and the way out differs:
  // splitting into two lines fixes the first and cannot fix the second.
  // Sharing one sentence would offer a remedy that does not work.
  //
  // ⚠️ AND write_off_needs_lot ABOVE IS NOW DEAD and stays mapped: 096 is
  // still in docs/sql — the record of what ran — and this file's own guard
  // reads that folder and demands a sentence for it. A sentence for a dead
  // code is cheaper than rewriting an executed script.
  insufficient_stock: 'products:stock.insufficientStock',
  write_off_not_outgoing: 'products:stock.writeOffNotOutgoing',

  // 🔴 مرآةُ الحارس فوقه، من ١٠١ — **وجملتُه تُكتب يومَ يُكتب الرمز.**
  //
  // ⚠️ **ورمزٌ بلا جملةٍ يصل الشاشةَ «صار خطأ غير متوقّع»** — وقعت فعلًا مع
  // `lot_insufficient`: رفضٌ صحيحٌ من القاعدة قُرئ عطلًا مجهولًا، **فذهب
  // التشخيصُ إلى الاستعلام وهو آخرُ مكانٍ فيه العطل.**
  return_not_outgoing: 'products:stock.returnNotOutgoing',
