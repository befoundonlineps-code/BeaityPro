...
describe('the product category payload', () => {
  ...
  it('carries NO storage of any kind — the link table is the only answer', () => {
    // 🔴 والعمودُ `product_categories.storage_id` يُسقَط بـ٠٨٨. الانتماءُ صفوفٌ
    // في `storage_categories` لأن المجلّدَ يمكن أن يكون في أكثر من مستودع، وهو
    // شرطُ النقل بين اثنين. **وآليّتان متوازيتان لمفهومٍ واحدٍ هي العلّةُ التي
    // دفعنا ثمنَها هنا بالذات.**
    //
    // ⚠️ **ومفاتيحُ الحمولة تُقارَن كقائمةٍ كاملة، لا بـ`not.toHaveProperty`.**
    // النفيُ يحرس اسمًا واحدًا يخطر بالبال؛ والقائمةُ تفشل مغلقةً على **أيّ**
    // مفتاحٍ ثالث — `storage_id` أو `storage_ids` أو ما يُخترع بعد شهر.
    //
    // ⚠️ **وهذه القاعدةُ عاشت جولةً كاملةً بتعليقٍ بلا حارس:** «`storage_id`
    // على الإنشاء وحدَه» كانت مكتوبةً في ثلاثة مواضع **ولا اختبارَ يمسّها** —
    // فحذفُها الآن ما كان ليُسقط شيئًا. التعليقُ يشرح لمن يقرأ، والاختبارُ
    // يحرس ممّن لا يقرأ، **ومن لا يقرأ هو من سيغيّرها.**
    for (const values of [
      { name: 'x' },
      { name: 'x', parentId: 'p' },
      { name: 'x', storageId: 'stor-1' },
      { name: 'x', storageIds: ['stor-1', 'stor-2'] },
    ]) {
      expect(Object.keys(productCategoryPayload(values)).sort()).toEqual(['name', 'parent_id'])
    }
  })
