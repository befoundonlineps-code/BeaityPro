...
export function validateStorage(values) {
  ...
  // pre-filled 100 and purchase_price, and storages were saved with them
  // untouched — a 100% wage deduction that nobody chose, in the owner's words
  // written to fill the box.
  //
  // 🔴 THAT IS A PAST EVENT, NOT THE STATE OF THE ROWS. This used to read
  // "both live storages carry a 100% deduction" in the present tense, and it
  // was measured false on 26 Aug 2026: script 110 §③ read every storage and
  // found FOUR of them, all with fine_percent AND fine_basis null — zero
  // policies, zero explicit zeros. The count was wrong too.
  //
  // The argument is untouched by that: a pre-filled field is still an answer
  // the screen gives on the user's behalf. Only the claim about today's rows
  // was stale — and it went stale silently, which is why it now carries the
  // date it was measured. This is unit_cost_required mirrored: there, an untouched box
