...
describe('a total built from a refused line is a refused total', () => {
  // ⚠️ THE OWNER FOUND THIS. Two rows each showing "—" while the panel below
  // read gross 200, line discounts −300, base −100. The rule "a refused line
  // shows no figures" had been applied to the line and not to the sum the line
  // is inside, so the withheld numbers were on screen anyway, added together.
  const refused = {
    productId: 'p2', enteredQuantity: '5', enteredUom: 'package',
    enteredUnitPrice: '50', lineDiscountKind: 'amount', lineDiscountValue: '300',
  }
  const honest = { ...refused, productId: 'p1', lineDiscountValue: '' }

  it('blocks the totals when any line is refused', () => {
    const { total, ladder, blocked } = documentTotals('supply', [honest, refused], {})
    expect(blocked).toBe(true)
    expect(total).toBeNull()
    // ⚠️ The ladder itself is still returned: the discount, freight and payment
    // controls live in that panel and have to stay usable while a row is fixed.
    // What the screen withholds is the figures, not the boxes.
    expect(ladder).not.toBeNull()
  })

  it('does not block on a document whose lines are all sound', () => {
    // The other direction, without which "blocked" could just be always true.
    const { total, blocked } = documentTotals('supply', [honest], {})
    expect(blocked).toBe(false)
    expect(total).toBe(250)
  })

  it('ignores a row still being typed, which has no product yet', () => {
    // Half a row is not a refused row. It takes no share of the split either —
    // the same rule, in the same place.
    const { blocked } = documentTotals('supply', [honest, { productId: '', bonusQuantity: '9' }], {})
    expect(blocked).toBe(false)
  })

  it('blocks on a bad bonus by the same route, not only on a discount', () => {
    const bonus = { ...honest, bonusQuantity: '9', lineDiscountValue: '' }
    expect(documentTotals('supply', [bonus], {}).blocked).toBe(true)
  })

  it('reports not blocked for a document that has no money at all', () => {
    // A write-off has no ladder to block, and `blocked` must not be undefined
    // there — the screen reads it on every doc type.
    expect(documentTotals('write_off', [honest], {}))
      .toMatchObject({ ladder: null, blocked: false })
  })
})
