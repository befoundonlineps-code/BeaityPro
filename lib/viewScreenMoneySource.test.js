...
const SOURCE = {
  ...
  // ⚠️ **والإرجاعُ يخزّنه للعبوة كذلك، مقيسًا لا مفترَضًا:**
  // `returnGrid.js:260` تضرب `perBase` في معامل التعبئة **قبل الحفظ**، فخانةُ
  // الشاشة للوحدة الأساسيّة والعمودُ للعبوة. ⇒ **الضربُ في `entered_quantity`
  // يعطي مبلغَ السطر بلا تحويل.**
  'ReturnDocumentView.js': 'entered_unit_price',
}
...
describe('🔴 مصدرُ رقمِ المال في شاشات العرض', () => {
  ...
  const ENTRY_OF = {
    ...
    'ReturnDocumentView.js': ['components/ReturnToSupplierScreen.js', /returnSupplier\.amountColumn/],
  }
  ...
  // ══════════════════════════════════════════════════════════════════
  // 🔴 خانةُ السعر وخليّةُ المبلغ من **معرِّفٍ واحد** — لا من مصدرين
  // ══════════════════════════════════════════════════════════════════
  //
  // ⚠️ **ثغرةٌ سمّاها المراجعُ في الحارس نفسِه:** ما فوق يعمل على **مستوى
  // الملفّ** — «لا `unit_cost` في ملفٍّ مُعلَنٍ `entered_unit_price`». **فلو
  // صارت خانةُ السعر تعرض مصدرًا ثالثًا ليس `unit_cost`** — سعرَ الكتالوج
  // `nominal_purchase_price` مثلًا — **مرّت صامتة**، والعمودان يقولان رقمين
  // لسؤالٍ واحد.
  //
  // ⇒ **وهو عينُ «التعدادُ يفشل مفتوحًا»** بلبوس المدى بدل القائمة.
  //
  // ⚠️ **والاسمُ يُشتقّ ولا يُكتب** — لو ثُبِّت `price` بالاسم لصار الحارسُ
  // يقرأ قائمةً يدويّةً من جديد.
  const PRICE_FIELD = {
    'SupplyDocumentView.js': 'entered_unit_price',
    'ReturnDocumentView.js': 'entered_unit_price',
  }

  it('🔴 وخانةُ السعر والمبلغُ من `const` واحدٍ — والاسمُ مشتقّ', () => {
    for (const [name, field] of Object.entries(PRICE_FIELD)) {
      const file = path.join(VIEW_DIR, name)
      expect(`${name} ⟵ موجود: ${fs.existsSync(file)}`).toBe(`${name} ⟵ موجود: true`)
      const body = strip(fs.readFileSync(file, 'utf8'))

      // ① قراءةٌ **واحدة** للحقل — واثنتان تعنيان مصدرين يقدران على الافتراق.
      const reads = (body.match(new RegExp(`\\b${field}\\b`, 'g')) || []).length
      expect(`${name} ⟵ قراءاتُ ${field}: ${reads}`).toBe(`${name} ⟵ قراءاتُ ${field}: 1`)

      // ② وهي في تصريحِ ثابتٍ يُشتقّ اسمُه منه.
      const decl = body.match(new RegExp(`const\\s+([A-Za-z_$][\\w$]*)\\s*=[^\\n]*\\b${field}\\b`))
      expect(`${name} ⟵ تصريحٌ باسم: ${decl ? decl[1] : 'لا تصريح'}`)
        .not.toBe(`${name} ⟵ تصريحٌ باسم: لا تصريح`)
      const id = decl[1]

      // ③ والمبلغُ يضرب في ذلك الاسم بعينه.
      //
      // ⚠️ **والحدُّ الأعلى مثبَّتٌ باسمه (`return`) لا بعددِ حروف** — نافذةٌ
      // بعرضٍ ثابتٍ هي بالضبط ما كسر حارسَ الإلغاء يومَ أُضيفت خاصّيّة.
      const amount = body.match(/const\s+amount\s*=[\s\S]*?\n\s*return\b/)
      expect(`${name} ⟵ تعبيرُ المبلغ مقروء: ${!!amount}`)
        .toBe(`${name} ⟵ تعبيرُ المبلغ مقروء: true`)
      expect(`${name} ⟵ المبلغُ يضرب في \`${id}\`: ${new RegExp(`\\b${id}\\b`).test(amount[0])}`)
        .toBe(`${name} ⟵ المبلغُ يضرب في \`${id}\`: true`)

      // ④ وخانةٌ ساكنةٌ ترسم ذلك الاسمَ بعينه — وهي خانةُ السعر.
      const boxes = body.match(/<StaticField[^>]*>[\s\S]{0,160}?<\/StaticField>/g) || []
      expect(`${name} ⟵ خاناتٌ ساكنةٌ مقروءة: ${boxes.length > 0}`)
        .toBe(`${name} ⟵ خاناتٌ ساكنةٌ مقروءة: true`)
      const shows = boxes.some((box) => new RegExp(`\\b${id}\\b`).test(box))
      expect(`${name} ⟵ خانةٌ ترسم \`${id}\`: ${shows}`)
        .toBe(`${name} ⟵ خانةٌ ترسم \`${id}\`: true`)

      // ⑤ ولا سعرَ كتالوجٍ في شاشة عرضٍ إطلاقًا — **ارتدادُ الإدخال يُحسم قبل
      //   الكتابة، فالمستندُ المرحَّلُ لا يستعير سعرَ اليوم.**
      expect(`${name} ⟵ سعرُ الكتالوج: ${/nominal_purchase_price|ORDER_PRICE_COLUMN|effectivePrice|priceBoxValue/.test(body)}`)
        .toBe(`${name} ⟵ سعرُ الكتالوج: false`)
    }
  })
})
