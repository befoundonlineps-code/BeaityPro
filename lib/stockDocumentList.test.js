...
describe('documentValue', () => {
  ...
  it('is zero for a transfer, and that is the true answer', () => {
    // Out of one storage and into another at the same stamped cost: the
    // document moved value, it did not create or destroy any.
    const rows = [
      { document_id: 'd1', product_id: 'p1', quantity_base: -75, unit_cost: 12.5, entered_uom: 'package' },
      { document_id: 'd1', product_id: 'p1', quantity_base: 75, unit_cost: 12.5, entered_uom: 'package' },
    ]
    expect(documentValue(rows, 'd1', productsById)).toBe(0)
  })
