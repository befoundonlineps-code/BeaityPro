...
const { INTERACTIVE, stripComments: strip } = require('./interactiveShapes')
...
describe('🔴 شاشاتُ العرض غيرُ تفاعليّةٍ بالبناء', () => {
  ...
  it('🔴 والبدائلُ الساكنةُ المشتركةُ محروسةٌ كذلك — وهي خارجُ المجلّد', () => {
    // ⚠️ **ثقبٌ لا يراه المسحُ:** `components/ref/RefStatic.js` هو ما يعطي
    // شاشاتِ العرضِ شكلَ الحقول والأزرار، **وهنّ الأربعُ يستوردنه** — وهو
    // **خارج** `components/documentView/` فلا تمشيه الحلقةُ أعلاه.
    // ⇒ **فـ`<button>` واحدٌ فيه يصل الأربعَ دفعةً واحدة**، والحارسُ أخضر.
    //
    // **وهي عينُ عائلة «قدرةٌ مركزيّةٌ ومستهلكٌ ما انربط فيها» معكوسةً:**
    // الحارسُ سليمٌ، **ومدًى واحدٌ يعيش خارجَه.**
    const shared = path.join(ROOT, 'components', 'ref', 'RefStatic.js')
    expect(`RefStatic موجود ⟵ ${fs.existsSync(shared)}`).toBe('RefStatic موجود ⟵ true')

    const body = strip(fs.readFileSync(shared, 'utf8'))
    for (const [label, shape] of FORBIDDEN) {
      expect(`RefStatic.js ⟵ ${label}: ${shape.test(body)}`)
        .toBe(`RefStatic.js ⟵ ${label}: false`)
    }

    // ⚠️ **وغيرُ مستوردٍ يعني حارسًا يحرس ملفًّا لا يستعمله أحد** — فالربطُ
    // مقيسٌ لا مفترَض.
