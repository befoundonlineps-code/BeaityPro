...
//
// 🔴 **وكانت الإبرةُ `<…Screen>` — أي لاحقةَ اسمٍ لا مفهومًا — فوقع ما تحذّر
// منه الفقرةُ أعلاه مرّةً ثانيةً بلبوسٍ آخر.**
//
// وُصلت `StocktakingSheet` في خانة `op === 'stocktake'`، **واسمُها ينتهي بـ
// `Sheet` لا `Screen`، فلم ترها الإبرةُ إطلاقًا** — لا خصائصُها فُحصت، ولا
// شيءَ أعلن أنها خارج المسح. ⚠️ **والذي أمسكها هو الحدُّ الأدنى وحدَه**
// (`>= 6` فصارت ٥ بخروج الشاشة القديمة) — **أي أن الالتقاط كان بأثرٍ جانبيّ،
// ولو دخلت شاشةٌ ثامنةٌ مع خروج لا شيءٍ لمرّت صامتة.**
//
// ⇒ **فالإبرةُ صارت على المفهوم: ما تركّبه الصفحةُ في خانةِ عمليّة** — أي
// `{op === '…' && ( <X …`. **واللاحقةُ لم تعد تعني شيئًا**، فمكوّنٌ يُسمّى
// `Sheet` أو `Panel` أو `Board` غدًا يدخل الفحصَ يومَ يُوصَل.
//
// ✅ **ووسّعت المدى من ستٍّ إلى عشر** — دخلت `StoragesManager` و
// `SuppliersManager` و`StockDocumentsList` و`StorageBalances` و
// `StocktakeCoverage`، **وكنّ خارج الحارس بلا سببٍ إلّا أسماءهنّ.**
const SCREENS = [...new Set(
  Array.from(PAGE.matchAll(/op === '\w+'\s*&&\s*\(\s*\n\s*<([A-Z][A-Za-z]*)/g)).map((m) => m[1])
)].sort()
...
describe('كلُّ خاصّيّةٍ تقرأها الشاشةُ تصلها من الصفحة', () => {
  it('يقرأ كلَّ شاشةٍ تركّبها الصفحةُ لا صفرًا — وإلّا مرّ الفحصُ على لا شيء', () => {
    ...
    // 🔴 **الأسماءُ قبل العدد، والترتيبُ مقصودٌ ومقيس.**
    //
    // كان العددُ أوّلًا، **فحين حُقنت الإبرةُ الضيّقةُ بيّنةً مضادّةً سقط الفحصُ
    // بـ«توقّعت ١٠ ووجدت ٥» ولم يُطبع اسمُ الشاشة الغائبة إطلاقًا** — ورقمٌ
    // مجرَّدٌ يدفع القارئَ إلى تعديل الرقم، **وهو بعينه ما تحذّر منه الفقرةُ
    // أسفله.** والاسمُ يقول أينَ ينظر.
    //
    // ⚠️ **و`StocktakingSheet` مثبَّتةٌ معهما لأنها هي التي كشفت عمى اللاحقة.**
    for (const pinned of ['WriteOffProductsScreen', 'ReturnToSupplierScreen', 'StocktakingSheet']) {
