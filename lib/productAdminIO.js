...
//
// ⚠️ This header used to open "there is no delete anywhere in this file". That
// was true when it was written and saveSetComponents falsified it a step later,
// with the comment left standing — and a comment that was true and became false
// is worse than none at all, because a missing one sends you to read the code
// while a wrong one excuses you from it. The irony is exact: the note that
// would have made somebody check what a delete returned was the same note
// saying there were no deletes to check.
//
// What it was reaching for is a rule about rows, not an inventory of this file:
//
//   A row a record points at is never deleted, only archived. products,
//   product_categories, storages and suppliers are all in this class —
//   stock_movements holds product_id with ON DELETE RESTRICT, so anything ever
//   moved could not be removed even if a policy allowed it. Archiving is the
//   only honest verb and the screens show no delete button at all.
//
//   A definitional row nothing points at is deleted outright, and the number
//   of rows deleted is checked. product_set_components is in this class: a
//   component is a line in a description, nothing refers to it, and somebody
//   who removes one means it gone rather than hidden.
...
// 🔴 أيُّ مستودعاتٍ يعيش فيها مجلّدٌ **أُنشئ للتوّ** — إدراجٌ فقط، بلا فرق.
//
// ⚠️ **إنشاءٌ لا تعديل، وذلك حدٌّ مقصود.** نقلُ مجلّدٍ بين المستودعات يجري من
// **نافذة المستودع** حيث تُؤشَّر الأصناف، لا من نافذة المجلّد. مكانان لقرارٍ
// واحدٍ هما المشكلةُ التي أنهيناها للتوّ، فلا تُفتح من الباب المقابل: هنا
// **إدراجٌ لروابطَ ما كانت موجودة**، ولا حذفَ ولا فرق.
//
// ⇒ ولذلك لا `keyedLinkDiff` هنا: الفرقُ يفترض حالةً سابقة، والمجلّدُ وُلد
// قبل سطرين.
//
// ⚠️ **ونداءٌ ثانٍ بعد إنشاء المجلّد، لا معاملةٌ واحدة** — البند ١ من
// «الكتابات» بـCLAUDE.md: الحالةُ الناقصة هنا **مجلّدٌ بلا روابط**، وهي
// **مرئيّةٌ** (يظهر تحت «كل المستودعات» بشارة «بلا مستودع») **وقابلةٌ للتصحيح**
// (يُؤشَّر من نافذة المستودع)، ولا يشتقّ منها أحدٌ رقمًا. وهو نفسُ صنف
// `product_set_components` بالضبط، لا صنف `stock_movements`.
export async function linkFolderToStorages(
  { categoryId, salonId, storageIds },
  client = supabase
) {
  const ids = [...new Set(storageIds || [])]
  if (ids.length === 0) return { ok: true, error: null }

  const rows = ids.map((storageId) => ({
    salon_id: salonId,
    storage_id: storageId,
    category_id: categoryId,
  }))

  // ⚠️ `seeded` لا تُذكر: افتراضُها `false` = قرارُ إنسان. ٠٦٦ب وحدَه كتب
  // `true` صراحةً للبذرة الأولى، وما لا يُذكر لا يُنسى.
