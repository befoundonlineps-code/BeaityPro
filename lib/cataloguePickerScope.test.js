...
describe('the row set cannot see the storage', () => {
  ...
  describe('the catalogue does not own a storage', () => {
    ...
    it('takes it from the page instead', () => {
      ...
      // And the page's lens is the one every other view reads — same variable,
      // resolved once.
      expect(page).toMatch(/const lensId = currentLens\(directories\.storages, chosenStorage, view\)/)
    })
  })
