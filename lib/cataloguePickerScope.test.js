...
describe('the row set cannot see the storage', () => {
  ...
  // ── One storage state, not two ──────────────────────────────────────────
  //
  // 🔴 THE CATALOGUE BRIEFLY HELD ITS OWN PICKER, with its own useState, while
  // the page had held a lens for six other views all along. Two controls for
  // one concept: set the lens to تجريبي, move to the catalogue, and the
  // catalogue says «all storages». One screen, two answers to one question.
  //
  // ⇒ Merged into the lens. And this is the guard the owner asked for by name,
  // because the merge is the kind of thing somebody undoes in three months
  // while "improving" something else and forgetting the link.
  //
  // ⚠️ It checks the SHAPE, not the behaviour: the component holds no storage
  // state at all, so there is nothing that CAN disagree. A test that set one
  // and read the other would prove they agree today; this proves the second
  // value does not exist.
  describe('the catalogue does not own a storage', () => {
    it('holds no storage state of its own', () => {
      const stateDeclarations = source.match(/useState\([^)]*\)/g) || []
      expect(stateDeclarations.length).toBeGreaterThan(3)
      // No `const [storageId, setStorageId] = useState(...)` — nor any alias.
      expect(source).not.toMatch(/\[\s*\w*[Ss]torage\w*\s*,\s*set\w*[Ss]torage\w*\s*\]\s*=\s*useState/)
    })

    it('renders no storage picker of its own', () => {
      // The other half: state could be avoided and a <select> still added,
      // writing through a callback. The lens is the page's control and there is
      // exactly one of it.
      const body = source.slice(source.indexOf('itemsToolbar='))
      expect(body).not.toMatch(/columns\.allStorages/)
      expect(body).not.toMatch(/<select[^>]*storage/i)
    })

    it('takes it from the page instead', () => {
      expect(source).toMatch(/storageId = ALL_STORAGES/)
      const page = fs.readFileSync(
        path.join(__dirname, '..', 'pages', 'products', 'index.js'), 'utf8',
      )
      const callSite = page.slice(
        page.indexOf('<ProductsBrowser'),
        page.indexOf('/>', page.indexOf('<ProductsBrowser')),
      )
      expect(callSite).toContain('storageId={lensId}')

      // And the page's lens is the one every operation reads — ONE call,
      // resolved once, off the page's own state.
      //
      // ⚠️ THE THIRD ARGUMENT USED TO BE `view` AND IS NOW THE LITERAL
      // 'catalog', and the change is the point rather than an accident. The tab
      // became a modal, so the catalogue is always what is underneath — and
      // passing the OPEN OPERATION here would renumber the grid behind the
      // modal the moment one opened, because currentLens turns «all storages»
      // back into one real storage for any view that may not widen.
      //
      // ⚠️ Written as «exactly one call, on these two values» rather than as
      // the literal line, because the literal is what broke here: a guard that
      // quotes a line of code fails on every rewording of it, including the
      // ones it was meant to allow. What must not come back is a SECOND lens.
      const lensCalls = page.match(/currentLens\(/g) || []
      expect(lensCalls.length).toBe(1)
      expect(page).toMatch(/const lensId = currentLens\(directories\.storages, chosenStorage,/)
      expect(page).not.toMatch(/currentLens\([^)]*,\s*op\s*\)/)
    })
  })

  it('is handed the balance load state, not just the rows', () => {
    // ⚠️ balancesLoading and balancesError DEFAULT to "loaded" in the component,
    // which fails OPEN: a caller that forgets them gets back the very lie the
    // four-state cell was written to stop — «ما تحرّك بعد» on a page whose
    // fetch is still out. There is exactly one call site, so the cheap way to
    // keep it honest is to read it.
    const page = fs.readFileSync(
      path.join(__dirname, '..', 'pages', 'products', 'index.js'), 'utf8',
    )
    const callSite = page.slice(
    ...
    expect(callSite).toContain('balances={balances.balances}')
    expect(callSite).toContain('balancesLoading={balances.loading}')
    expect(callSite).toContain('balancesError={balances.error}')
  })
  ...
  it('and the row set really is blind to it, not merely unnamed', () => {
    ...
    // ⚠️ A FOLDER IS CHOSEN NOW, AND THAT IS THE RULE CHANGE RATHER THAN A
    // REPAIR. «No folder» used to mean every product; the owner has since set
    // the reference's rule — an empty grid until a folder is picked — so the
    // old arguments produced an empty list on BOTH sides and the comparison
    // compared nothing to nothing. A test whose two sides are empty passes
    // loudest of all.
    const args = { products, categories, categoryId: 'c1', search: '', hideArchived: false }

    const asAll = catalogueRows(args)
    const asOneStorage = catalogueRows({ ...args, storageId: 's1', balances: [] })

    // The claim is unchanged: a storage handed to this function changes
    // nothing, because nothing in it reads one. «بلسم» has zero movements
    // anywhere, so it is the first row a leak would take.
    expect(asAll).not.toHaveLength(0)
