...
describe('which screens refuse to be entered without a storage', () => {
  ...
  it('blocks the order screen too, and NOT because its row gained a column', () => {
    // 🔴 THIS ASSERTION REVERSED, AND THE OLD ONE WAS RIGHT WHEN IT WAS WRITTEN.
    // It read «the orders screen is not about a place at all» — true of the ROW
    // and false of the SCREEN once it was rebuilt to the reference: its first
    // window asks which of THIS storage's folders to order from, and its grid
    // shows what is in stock HERE. Neither has an answer at «all storages».
    //
    // ⚠️ AND THE SCHEMA DID NOT MOVE. product_orders still has no storage
    // column and isStorageScoped('order') is still false — asserted next door in
    // storageScopedOperations.test.js, deliberately left alone. The storage is a
    // LENS here, never a column, so the two files disagree about `order` on
    // purpose and each is answering its own question.
    expect(navigationBlocked('orders', ALL_STORAGES)).toBe(true)
    expect(navigationBlocked('orders', 's1')).toBe(false)
  })

  it('lets the rest through at any lens', () => {
    // The catalogue and the document list can mean «all»; the storages manager
    // and the suppliers list are not about a place at all.
    for (const view of ['catalog', 'documents', 'storages', 'suppliers']) {
