...
export async function loadOccupancy({ employeeId, start, end }) {
  ...
  const [employeeRes, unitRes] = await Promise.all([
    ...
    overlapping(
      supabase
        .from('appointments')
        .select('id, resource_unit_id, start_time, end_time, status')
...
// The same thing for a whole session at once: every participant's own rows,
// grouped by employee, plus the single global set of unit-holding rows.
//
// One query covers all the employees rather than one per participant — the
// window is identical for all of them, so asking separately would only
// multiply round-trips.
export async function loadGroupOccupancy({ employeeIds, start, end }) {
  ...
  const [employeeRes, unitRes] = await Promise.all([
    ids.length
      ? overlapping(
          supabase.from('appointments').select('id, employee_id, start_time, end_time, status').in('employee_id', ids)
        )
      : Promise.resolve({ data: [], error: null }),
    overlapping(
      supabase
  ...
  for (const id of ids) rowsByEmployee[id] = []
  for (const row of employeeRes.data || []) {
    if (rowsByEmployee[row.employee_id]) rowsByEmployee[row.employee_id].push(row)
  }

  return { rowsByEmployee, unitRows: unitRes.data || [] }
}
