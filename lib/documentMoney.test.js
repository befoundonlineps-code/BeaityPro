import {
  ...
  lineGross, lineDiscountAmount, lineNet, validateLineMoney, paidQuantity, lineDisplay,
...
describe('what a line shows while it is being typed', () => {
  const row = (over) => ({
    productId: 'p1', enteredQuantity: '7', enteredUom: 'package',
    enteredUnitPrice: '50', lineDiscountKind: 'percent', lineDiscountValue: '', ...over,
  })

  it('shows both figures on a clean line, the per-unit landed across the delivery', () => {
    expect(lineDisplay(row({ bonusQuantity: '1' })))
      .toEqual({ error: '', net: 300, perUnit: 300 / 7 })
  })

  // ⚠️ THE ONE THE OWNER SAW. A bonus of 8 on 7 gives −50 and −7.14: true
  // arithmetic about a document that will be refused. Both are withheld, and a
  // sentence takes their place — because a negative net is this line's WEIGHT
  // in the split and would drag every other line's share with it.
  it('withholds both figures on a refused line and names the reason', () => {
    expect(lineDisplay(row({ bonusQuantity: '8' })))
      .toEqual({ error: 'products:money.bonusOverQuantity', net: null, perUnit: null })
  })

  it('stays silent until the line has produced a figure', () => {
    // Typing the bonus first must not start an argument about a quantity that
    // is not there yet. The save-time refusal still covers it.
    expect(lineDisplay(row({ enteredQuantity: '', bonusQuantity: '3' })))
      .toEqual({ error: '', net: null, perUnit: null })
  })

  it('covers the discount by the same rule, not only the bonus', () => {
    // ⚠️ The rule is "a refused line shows no figures", not "a bonus does". A
    // line discount larger than the line produces exactly the same negative
    // net, and predates the bonus by a stage.
    expect(lineDisplay(row({ lineDiscountKind: 'amount', lineDiscountValue: '400' })))
      .toEqual({ error: 'products:money.lineDiscountOverLine', net: null, perUnit: null })
  })

  it('says nothing about an untouched row', () => {
    expect(lineDisplay({ productId: '', enteredQuantity: '', enteredUnitPrice: '' }))
      .toEqual({ error: '', net: null, perUnit: null })
  })
})
