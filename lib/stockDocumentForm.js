...
import { validateDocumentMoney, allocateDocument, validateLineMoney } from './documentMoney'
...
// A blank line on a document screen.
//
// ⚠️ HERE RATHER THAN IN THE SCREEN, because it stopped having one author. The
// screen builds these when somebody presses "add line", and lib/productOrder.js
// builds them when an order pre-fills a supply — and a row missing a key the
// screen expects is not an error, it is a select whose value matches no option
// and quietly shows a different one. Two copies of a shape agree the day they
// are written (ADR-053).
//
...
// document discount and the freight have landed on them. Two different numbers,
// and neither can be recovered from the other.
//
// ⚠️ Blank quantities and a blank bonus, never 0. The whole module treats an
// untouched box as a statement nobody made, and a stored zero would say "a
// bonus of nothing" — a different row from "no bonus" for anybody counting free
// goods.
...
export function documentTotals(docType, rows, money) {
  ...
  if (!form || !form.money) return { lineCount, total: null, ladder: null, blocked: false }
  ...
  // ⚠️ A TOTAL BUILT FROM A REFUSED LINE IS A REFUSED TOTAL. The owner found
  // this: two rows each showing "—" while the panel underneath read gross 200,
  // line discounts −300, base −100. The rule "a refused line shows no figures"
  // was applied to the line and not to the sum the line is inside — so the very
  // numbers being withheld one row up were on screen one row down, added
  // together.
  //
  // And −100 is not merely ugly. It is documentDiscountBase, the figure the
  // document discount is measured against and divided by, so every rung below
  // it is describing a document that cannot be posted.
  //
  // The ladder is still returned: the discount, freight and payment boxes live
  // inside that panel and must stay usable while a line is being fixed. What
  // the screen withholds is the figures, not the controls.
  const blocked = (rows || []).some((row) => row && row.productId && validateLineMoney(row))
  return { lineCount, total: blocked ? null : ladder.net, ladder, blocked }
}
...
//
// ⚠️ This paragraph used to read «a rule written for a screen that does not
// exist yet is a rule nobody applies — post_stocktake and stocktakeLines have
// both been sitting unused», naming items 49 and 50, where the engine ran ahead
// of the screens.
//
// 🔴 THE SCREEN EXISTS AND USES BOTH: components/StocktakeScreen.js, mounted at
// pages/products/index.js under `view === 'stocktake'`, and lib/stocktakeSheet.js
// beside it. The door was built and somebody walked through it.
//
// ⇒ So the reason survives and its tense does not: the rule is shared through a
// second door rather than a wider one, and it is APPLIED rather than waiting to
// be. Found by lib/staleTemporaryComments.test.js — the sweep that used to be a
// habit.
//
