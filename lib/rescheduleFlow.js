...
import { raisedCode, rpcErrorKey } from './rpcErrors'
...
export function rescheduleErrorKey({ error, kind, exhausted, data }) {
  if (error) {
    ...
    // The dialog disables confirm until a reason is picked, so the missing
    // reason is a backstop rather than a path a receptionist reaches — and it
    // wants this dialog's wording, not the general one, which is what the
    // override is for.
    if (raisedCode(error)) {
      return rpcErrorKey(error, {
        reschedule_reason_required: 'appointments:rescheduleDialog.reasonRequiredError',
      })
    }
