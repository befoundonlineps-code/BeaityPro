...
// What the document's rows become once an order has been poured into them.
//
// ⚠️ OUT HERE RATHER THAN INSIDE THE SCREEN, because it is the one operation in
// this feature that can destroy something a person typed — and a merge written
// inside a setState callback is invisible to every test until the day somebody
// reports losing three lines they cannot prove they wrote.
//
// Appending DROPS the untouched blanks rather than keeping them, and that is
// not tidying: a blank row between the kept lines and the poured ones reads as
// a line somebody started, and the row after a blank is where people stop
// looking. What is kept is work; what is dropped never was.
export function mergeFilledRows(existing, picked, replace) {
  const kept = replace ? [] : (existing || []).filter((row) => row && row.productId)
  return [...kept, ...(picked || [])]
}
