...
describe('the window is wired to the rule, and the page feeds it', () => {
  ...
  it('refreshes BOTH loaders after a write — one refresher, no choice', () => {
    // 🔴 العطلُ اللي وقع، ووصل شاشةَ بشّار: مجلّدان انولدوا بشارة «بلا مستودع»
    // **ورابطُهما مكتوبٌ بالقاعدة.**
    //
    // كتابةُ المجلّد تمسّ جدولين يملكهما خطّافان — `product_categories` في
    // `useProductCatalog` و`storage_categories` في `useInventoryDirectories` —
    // و`onSaved` كان `catalogue.reload` وحدَها. فظهر المجلّدُ ولم يظهر رابطُه،
    // **وقرأت الشاشةُ غيابَ الرابط عن نسختها القديمة «ما في رابط».**
    //
    // ⚠️ **وما من اختبارٍ سابقٍ كان يقدر يمسكه**: كلُّها تمرّر `storageCategories`
    // خاصّيّةً جاهزة، فما في واحدٍ منها يمرّ بـ«شو بيصير **بعد** الكتابة».
    // اختبارُ الدالّة النقيّة عمياءُ عن التحديث بنيويًّا.
    //
    // ⇒ فالحارسُ على **الشكل**: كلُّ `onSaved` بالشاشة **نفسُ الاسم**، وتعريفُ
    // ذاك الاسم ينادي المُحدِّثَين. نافذةٌ جديدةٌ بمُحدِّثٍ آخر تُسقطه، ومُحدِّثٌ
    // ينسى أحدَهما يُسقطه.
    const browser = fs.readFileSync(path.join(ROOT, 'components', 'ProductsBrowser.js'), 'utf8')
    const handlers = [...browser.matchAll(/onSaved=\{(\w+)\}/g)].map((m) => m[1])

    expect(handlers.length).toBeGreaterThan(0)
    expect([...new Set(handlers)]).toHaveLength(1)

    const definition = new RegExp(`const ${handlers[0]} = \\(\\) => \\{([^}]*)\\}`).exec(browser)
    expect(definition).toBeTruthy()
    expect(definition[1]).toContain('catalogue.reload()')
    expect(definition[1]).toContain('directories.reload()')
  })

  it('takes the directory as one object, so data and refresher cannot be split', () => {
    // ⚠️ **البنيةُ هي الإصلاح، والحارسُ فوقها فقط.** لمّا تصل المصفوفةُ
    // ودالّتُها من كائنٍ واحد، ما بيقدر منادٍ يعطي وحدةً وينسى التانية —
    // وهاد اللي وقع بالضبط: `storageCategories={directories.storageCategories}`
    // بلا `directories.reload`.
    const browser = fs.readFileSync(path.join(ROOT, 'components', 'ProductsBrowser.js'), 'utf8')
    expect(browser).toMatch(/const \{[^}]*storageCategories[^}]*\} = directories/)

    // ⚠️ **مقصورٌ على نداء `ProductsBrowser` وحدَه، وذلك دقّةٌ لا تساهل.**
    // `StoragesManager` يأخذ المصفوفةَ خاصّيّةً **ومعها `reload={directories.reload}`
    // ويستعملها `onSaved`** — فالزوجُ عنده مجتمعٌ فعلًا وإن كان مفكوكَ الشكل.
    // حارسٌ يشمله يسقط على شيءٍ سليم، **وحارسٌ يصرخ على البريء يُعدَّل رقمُه
    // حتى يخفي يومًا حالةً حقيقيّة.**
    const at = page.indexOf('<ProductsBrowser')
    expect(at).toBeGreaterThan(-1)
    const element = page.slice(at, page.indexOf('/>', at))
    expect(element).toContain('directories={directories}')
    for (const array of ['storageCategories', 'storages', 'suppliers']) {
      expect(element).not.toContain(`${array}={directories.${array}}`)
    }
  })
