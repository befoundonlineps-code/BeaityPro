...
//
// The other half was reported measured against the live database (storage_id
// NOT NULL on both tables, read from the catalogue rather than from a script).
// ⚠️ That measurement is NOT in this repository — no 067 file exists here — so
// it is second-hand to this guard and, either way, a reading of one moment
// rather than a standing promise. If the schema drifts, the thing that will
// notice is another live read, not this file.
...
describe('the storage-scoped map matches the schema it claims to describe', () => {
  ...
  it.each(Object.keys(TABLE_NEEDS_STORAGE))('%s', (table) => {
    ...
    expect(hasMandatoryStorage(block.body)).toBe(TABLE_NEEDS_STORAGE[table])
  })
