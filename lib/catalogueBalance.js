...
export function catalogueBalanceRows({ balances, products, storageId = ALL_STORAGES }) {
  if (storageId) {
    return new Map(
      balanceRows({ balances, products, storageId })
        // ⚠️ lowSupply is dropped here too, not only in the merge — see
        // LOW_SUPPLY_GRAIN at the foot of this file. balanceRows keeps
        // computing it for the screens that already draw it; this column
        // declines to add a third reading of an undecided number.
        .map((row) => [row.product.id, { ...row, lowSupply: false, needsAttention: false }])
    )
  }
