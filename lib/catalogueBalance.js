...
// What a product holds ANYWHERE ELSE than the storage being shown.
//
// 🔴 THE REFERENCE HAS THIS COLUMN AND WE DID NOT, and it is the answer to the
// question the balance column raises rather than settles: «zero here» is only
// half a sentence, and the other half decides whether somebody orders more or
// walks to the next room. design/TOKENS.md already wrote the rule down — the
// grey zero is calm precisely because the link beside it says what to do — and
// the link had nowhere to point until this existed.
//
// ⚠️ NULL, NOT ZERO, WHEN THE QUESTION CANNOT BE ASKED. Showing «all storages»
// means there is no elsewhere, and a 0 in that column would answer a question
// nobody asked with a number that looks measured. The caller draws nothing.
//
// ⚠️ And it counts every storage found in the rows rather than every storage in
// the list, for the reason storageIdsIn already gives: a balance row for a
// storage nobody listed still holds stock, and omitting it makes «elsewhere»
// quietly short — which is the direction that sends somebody to order goods
// they already own.
export function otherStorageTotals({ balances, storageId }) {
  const totals = new Map()
  if (!storageId) return totals
