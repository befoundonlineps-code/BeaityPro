...
//
// 🔴 A THIRD ENTRY WAS HERE AND IS GONE — `stocktakeInputMethods` — and the
// reasoning it carried is reversed rather than deleted, because it was right
// about the thing it was arguing against.
//
// It said: «A caret revealing one entry is a menu that lies about having a
// choice.» True of a caret that reveals ONE entry. `StocktakeMethodMenu` now
// draws all three — by hand live, barcode and Excel rendered STRUCTURALLY
// disabled with the reason on each, no event handler anywhere near them
// (guarded by lib/stocktakeScreenShape.test.js).
//
// ⇒ An option disabled for a stated reason is an absence DECLARED, not a choice
// falsified. The person sees the reference has three routes, that we built one,
// and reads why — which is truer than hiding two and letting them hunt for a
// feature they believe exists.
