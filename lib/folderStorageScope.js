...
// 🔴 الانتماءُ صار في `storage_categories` لا في عمودٍ على المجلّد — والسببُ
// ليس تفضيلًا بل **قيدٌ لا يمكن التعبير عنه بغيره.**
//
// المطلوب: «كلُّ مستودعٍ له أصنافٌ محدَّدةٌ مسموحٌ تكون فيه». وشرطُ النقل بين
// مستودعين أن يحفظ الاثنان **نفسَ المجلّد**. وتحت «مجلّد = مستودعٌ واحد» تصير
// تلك الجملةُ «الوجهةُ = مستودعُه الوحيد» — **أي منعَ كلِّ نقل.**
//
// ⚠️ **وهذا الجدولُ سبق العمودَ ولم يُرَ.** ٠٦٦أ بناه ورفض العمودَ بالاسم:
// «neither product_categories.storage_id (refused by a folder in two
// storages)» — على بيّنةٍ مقيسة: مستودعٌ ثالثٌ أُشّرت له نفسُ ثلاثة مجلّدات
// مستودعٍ آخر. ثمّ بُني العمودُ في ٠٨٥ لأن ٠٨٤ سأل عن **أعمدة المجلّد وقيوده**،
// وقيدُ جدول الربط يعيش على جدول الربط. والسؤالُ الذي يجده هو «مَن **يشير
// إلى** `product_categories`» — وهو القسم A من ٠٨٧_١ بالحرف.
//
// مجموعةُ المجلّدات التي يحفظها مستودعٌ بعينه.
export function linkedCategoryIds(links, storageId) {
  const ids = new Set()
...
// ومجموعةُ كلِّ مجلّدٍ يحفظه **أيُّ** مستودع — للتمييز بين «غيرِ مربوطٍ بعد»
// و«مربوطٍ بمكانٍ آخر».
function anyLinkedIds(links) {
  const ids = new Set()
  for (const link of links || []) ids.add(link.category_id)
  return ids
}
...
// read as covered.
export function foldersForStorage(categories, storageId, links) {
  ...
  // ⚠️ «المُسنَد» صار «المربوط»، والبقيّةُ لم تتغيّر حرفًا — العمودُ الفقريّ
  // والموقفان والترشيحُ النهائيّ كما هي. **وهذا هو العائدُ كلُّه:** التغييرُ
  // البنيويّ من عمودٍ إلى جدولِ ربطٍ مسّ **سطرًا واحدًا** هنا، والشجرةُ
  // والعقدةُ المعلَّقة والبحثُ والحالاتُ الفارغة لم يمسّها شيء، لأنها كلَّها
  // مشتقّةٌ من هذه الدالّة.
  const linked = linkedCategoryIds(links, storageId)
  const assigned = all.filter((c) => linked.has(c.id))
...
export function isPassThroughFolder(category, storageId, links) {
  ...
  return !linkedCategoryIds(links, storageId).has(category.id)
}

// المجلّداتُ التي لا يحفظها **أيُّ** مستودع، كي تشير الشاشةُ إليها.
//
// ⚠️ **والمعنى تغيّر مع النموذج، والاسمُ بقي:** كان «العمودُ فارغ»، وصار «ولا
// رابطَ له في أيّ مستودع». والحالتان تُرسمان بنفس الطريقة — تحت «كل
// المستودعات» وحدها — لكن الثانية **مرئيّةٌ وقابلةٌ للإصلاح من نافذة المستودع**،
// والأولى كانت تحتاج سكربتًا.
//
...
export function unassignedFolders(categories, links) {
  const linked = anyLinkedIds(links)
  return (categories || []).filter((c) => !linked.has(c.id))
}

export function isUnassignedFolder(category, links) {
  return !!category && !anyLinkedIds(links).has(category.id)
}
