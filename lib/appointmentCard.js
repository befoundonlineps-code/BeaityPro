...
// "09:00 – 10:00" from a booking's own span. No rounding and no timezone
// work: these are the same Date values the block is positioned from, so the
// label and the position can never disagree.
export function timeRangeLabel(appointment) {
  ...
  return `${at(appointment.start_time)} – ${at(appointment.end_time)}`
}
