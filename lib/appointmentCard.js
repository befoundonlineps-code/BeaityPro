...
export function approveErrorKey({ error, data }) {
  if (error) {
    // Only a code we raise deliberately answers here. Anything else returns
    // null so the caller reports it, which keeps this function about what the
    // card says and not about every way a write can fail.
    if (raisedCode(error)) return rpcErrorKey(error)
