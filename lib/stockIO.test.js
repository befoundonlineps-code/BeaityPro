...
describe('postStockDocument', () => {
  it('calls the function by name with every parameter', () => {
    ...
    expect(rpc).toHaveBeenCalledWith('post_stock_document', {
      ...
      // ⚠️ Sent explicitly even when empty. PostgREST matches an overload by
      // the parameters present, and a missing one can pick a different
      // signature — which is exactly the ambiguity the DROP in 050c removed.
      p_discount_kind: null,
      p_discount_value: null,
      p_transport_amount: null,
      p_transport_paid_to: null,
      p_paid_amount: null,
      p_payment_method: null,
      // ⚠️ ٠٩٩. **وهذا الحارسُ عضّ لحظةَ إضافته**، وهو المقصود منه: حمولةٌ
      // مثبَّتةٌ حرفًا تسقط عند أوّل معاملٍ يُضاف، **فيقف عليه إنسانٌ مرّةً** بدل
      // أن يمرّ معاملٌ منسيٌّ ويُقرأ غيابُه افتراضًا صامتًا.
      //
      // 🔴 **ويُرسَل ولو فارغًا كأخواته، والسببُ هنا أشدّ:** ٠٩٩ حذف التوقيعَ
      // القديمَ صراحةً، **فحمولةٌ بلا هذا الاسم لا تجد دالّةً تطابقها إطلاقًا**
      // — لا نسخةً قديمةً تلتقطها.
      p_doc_number: null,
    })
  })
