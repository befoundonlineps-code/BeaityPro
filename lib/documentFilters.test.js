...
describe('which documents can carry a supplier at all', () => {
  it('is exactly the rows with a counterparty outside the salon', () => {
    ...
    //
    // 🔴 **وصارت ثلاثةً بدخول الطلبيّة، والعنوانُ تغيّر معها من «الاثنين» إلى
    // «الصفوف» — فالعددُ لم يكن الادّعاءَ يومًا، والصفةُ هي.**
    //
    // للطلبيّة `supplier_id` حقيقيّ ⇒ **فبدونها يُعطَّل مرشِّحُ المورّد لحظةَ
    // اختيار «طلب بضاعة»**، وهو الكذبُ الذي وُجدت `supplierFilterApplies`
    // لمنعه. ⚠️ **وهي ليست نوعَ مستندٍ ولا تُرحَّل**، فدخولُها هنا لا يقترب من
    // `post_stock_document` — والقوائمُ الثلاثُ التي تغذّيه تستبعدها عمدًا.
    expect(SUPPLIER_TYPES).toEqual(['supply', 'return_to_supplier', 'order'])
    expect(hasSupplier('supply')).toBe(true)
    expect(hasSupplier('order')).toBe(true)
    expect(hasSupplier('transfer')).toBe(false)
    expect(hasSupplier('stocktake')).toBe(false)
    expect(hasSupplier('write_off')).toBe(false)

    // 🔴 **والحدُّ يُحرَس هنا لا يُترك لتعليق:** «طلب بضاعة» **خارج** الثلاث
    // التي تُرحَّل — وإضافتُها إلى أيٍّ منها تفتح بابَ ترحيل طلبيّةٍ حركةً
    // فعليّةً في المخزون. **وهذا أخطرُ ما في دمج المصدرين.**
    // eslint-disable-next-line global-require
    const { RECEIPT_TYPES, ISSUE_TYPES, OWN_FUNCTION, ORDER_DOC_TYPE } = require('./stockDocument')
    for (const [name, list] of [
      ['RECEIPT_TYPES', RECEIPT_TYPES], ['ISSUE_TYPES', ISSUE_TYPES], ['OWN_FUNCTION', OWN_FUNCTION],
    ]) {
      expect(`${name} فيها order ⟵ ${list.includes(ORDER_DOC_TYPE)}`)
        .toBe(`${name} فيها order ⟵ false`)
    }
  })
