...
describe('an empty result says which kind of empty it is', () => {
  ...
  it('names a contradictory ask rather than drawing a blank list', () => {
    // ⚠️ Asking a supplier of documents that have none returns nothing FOREVER.
    // Drawn as a plain empty list it invites the conclusion that the documents
    // are missing. Only reachable with "all types", which is why the disabled
    // control cannot catch it.
    ...
    })).toBe(FILTER_EMPTY.CONTRADICTORY)
  })
