...
describe('which storage the document list is showing', () => {
  it('follows the lens', () => {
    expect(storageInForce('s1', false)).toBe('s1')
    expect(storageInForce('s2', false)).toBe('s2')
  })

  it('looks past it only when asked to', () => {
    ...
    expect(storageInForce('s1', true)).toBe('')
  })
  ...
  it('shows everything when there is no lens to follow', () => {
    ...
    expect(storageInForce('', false)).toBe('')
    expect(storageInForce(null, false)).toBe('')
    expect(storageInForce(undefined, false)).toBe('')
  })

  it('is a function of the lens and the toggle, and of nothing remembered', () => {
    // ⚠️ The property the first attempt broke. Two calls with the same lens and
    // toggle give the same answer whatever happened in between, which is what
    // "derived, not copied" means when written as a test rather than a comment.
    expect(storageInForce('s1', false)).toBe(storageInForce('s1', false))
    expect(storageInForce('s9', false)).toBe('s9')
  })
})
