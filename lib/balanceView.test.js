...
describe('costIsEstimated — بند ٣٤، والعمودُ حيٌّ منذ ٠٤٣', () => {
  it('is FALSE when the column is not there, and invents nothing', () => {
    // 🔴 **وكان هنا سطرٌ يقول «The script is prepared and not run … the badge
    // stays dark» — صحّ يومَ كُتب وصار كذبًا بصمت.** `docs/sql/README.md:13`
    // يقول `٠٤٣ ✅ شُغّل`، و`product_balances` تحمل `cost_has_estimate` منذ
    // ذلك اليوم (`043-cost-estimated.sql:167`).
    //
    // ⚠️ **وضررُه عمليٌّ لا شكليّ:** يُقرأ «الشارةُ مطفأةٌ اليوم»، **فمن يبني
    // شاشةً لا يعرض تكلفةً مقدَّرةً لأنه يظنّها بلا مصدر** — وهي مضاءةٌ فعلًا.
    // والسلوكُ كان سليمًا في الحالتين؛ **الكاذبُ التعليقُ وحدَه.**
    //
    // ✅ **وما تحرسه هذه الحالاتُ باقٍ ولم يتغيّر:** صفٌّ بلا العمود إطلاقًا —
    // تجهيزةٌ قديمة، أو نتيجةُ استعلامٍ لا يختاره — **لا يُخترع له تقدير.**
