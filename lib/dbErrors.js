...
// `overrides` is how one raised code says different things on different
// screens. appointment_not_cancellable means "this booking cannot be
// cancelled" in the actions dialog and "your view is out of date" in the
// day-status dialogs, where somebody is releasing a whole day and a booking
// that moved underneath them is stale news rather than a refusal.
export function dbErrorSentence(error, t, context, overrides) {
  if (!error) return null

  // A raised code first, because it is the more specific of the two — then the
  // SQLSTATE.
  //
  // ⚠️ SQLSTATE overrides exist for a case the global map cannot serve: a
  // unique violation is 23505 wherever it happens, and only the caller knows
  // WHICH uniqueness was violated and what that means to a person. On the
  // reversal screen it means "somebody reversed this a second ago", which is
  // news rather than a fault — and the generic "that already exists" is
  // nearly useless there.
  //
  // ⚠️ It is deliberately NOT a global table of constraint names. Constraints
  // are added over time and such a list never closes — the "inventory not
  // rule" fault wearing a message map (item 56). A caller naming the failure
  // it understands is bounded by construction: the sentence lives with the
  // screen that knows why.
  const code = raisedCode(error)
  const override = overrides && (overrides[code] || overrides[error.code])
  if (override) {
