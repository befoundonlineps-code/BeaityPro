...
import { BALANCE_STATE } from './balanceView'
...
describe('one storage', () => {
  ...
  it('keeps the low-supply signal balanceRows computes', () => {
    const rows = catalogueBalanceRows({
...
describe('all storages', () => {
  ...
  it('stays silent about low supply, and that is a decision not a gap', () => {
    // ⚠️ low_supply_units is one number on the product and stock sits in
    // several storages, so "this shelf is low" and "the salon is low" disagree
    // about real rows. A badge meaning one thing in one dropdown position and
    // another in the next is the fault this project keeps naming. Silence
    // claims nothing; picking a meaning nobody chose does.
    const rows = catalogueBalanceRows({
