import { catalogueBalanceRows, otherStorageTotals, ALL_STORAGES } from './catalogueBalance'
...
describe('all storages', () => {
  ...
  it('adds up every storage except the one on screen', () => {
    const rows = otherStorageTotals({
      balances: [bal('p1', 's1', 75), bal('p1', 's2', 25), bal('p1', 's3', 4), bal('p2', 's1', 9)],
      storageId: 's1',
    })
    expect(rows.get('p1')).toBe(29)
    // p2 lives only in the storage being shown, so «elsewhere» has nothing to
    // say about it — and no entry is what «nothing to say» looks like.
    expect(rows.has('p2')).toBe(false)
  })

  it('counts a storage nobody listed, because the stock is still there', () => {
    // The same reason storageIdsIn derives from the data: a row for a storage
    // missing from the directory still holds goods, and leaving it out makes
    // «elsewhere» short — which sends somebody to order what they already own.
    expect(otherStorageTotals({ balances: [bal('p1', 'ghost', 12)], storageId: 's1' }).get('p1')).toBe(12)
  })

  it('says nothing about «elsewhere» when there is no elsewhere', () => {
    // 🔴 The empty map, not a map of zeros. Standing on «all storages» there is
    // no other storage to be in, and a column reading 0 there would answer a
    // question nobody asked with a figure that looks measured — «you have none
    // anywhere else» about a product sitting in four rooms.
    expect(otherStorageTotals({ balances, storageId: ALL_STORAGES }).size).toBe(0)
    expect(otherStorageTotals({ balances, storageId: null }).size).toBe(0)
    expect(otherStorageTotals({ balances, storageId: '' }).size).toBe(0)
  })
