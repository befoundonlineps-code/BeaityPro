...
describe('buildPricingMatrix and archived folders', () => {
  it('leaves an archived folder out of the matrix entirely', () => {
    const cats = [
      { id: 'c1', name: 'الشعر', parent_id: null, pricing_role: 'stylist', sort_order: 1, is_active: true },
      { id: 'c2', name: 'المكياج', parent_id: null, pricing_role: 'makeup_artist', sort_order: 2, is_active: false },
    ]
    const svcs = [
      { id: 's1', category_id: 'c1', name: 'قص', price: 60, is_active: true, sort_order: 1 },
      { id: 's2', category_id: 'c2', name: 'سهرة', price: 200, is_active: true, sort_order: 1 },
    ]
    const groups = buildPricingMatrix({ categories: cats, services: svcs })
    expect(groups.map((g) => g.root.name)).toEqual(['الشعر'])
  })
})
