...
export async function postStockDocument(
  {
    ...
    paidAmount, paymentMethod, docNumber,
  },
  client = supabase
) {
  const { data, error } = await client.rpc('post_stock_document', {
    ...
    // 🔴 **رقمُ مستندنا نحن، لا رقمُ ورقة المورّد** (`supplier_doc_number` فوق).
    //
    // ⚠️ **وهذا المعاملُ يخالف أخواتِه في نقطةٍ تستحقّ القول:** التعليقُ أعلاه
    // يقول إن النداءَ «يعمل قبل الهجرة وبعدها» — **وذلك صحيحٌ عن عمودٍ يُضاف
    // ولا يُذكر في النداء، وخاطئٌ عن معامل.** PostgREST يحلّ الدالّةَ **بأسماء
    // المعاملات**، فإرسالُ اسمٍ لا وجودَ له يفشل بـ`PGRST202` — **والنداءُ كلُّه
    // يسقط، لا هذا الحقلُ وحدَه.**
    //
    // ⇒ **فهذا السطرُ يلزمه ٠٩٩ منفَّذًا قبله**، ولا يحتمل الترتيبَ المعاكس.
