...
import { storageInForce } from './storageScope'
import { ORDER_DOC_TYPE } from './stockDocument'

// 🔴 **والطلبيّةُ ثالثتُها، ووجودُها هنا انكشف أثناء البناء لا بتخطيط.**
//
// لها `supplier_id` حقيقيّ ⇒ **فبدونها يُعطَّل مرشِّحُ المورّد لحظةَ اختيار
// «طلب بضاعة»** — وهو الكذبُ الذي بُني `supplierFilterApplies` لمنعه: مرشِّحٌ
// يُهمَل بصمتٍ أسوأُ من مرشِّحٍ يقول «ليس هنا».
//
// ⚠️ **وهي القائمةُ الوحيدةُ من إحدى عشرةَ التي احتاجت الطلبيّةَ ولم تكن
// متوقَّعة** — مسحٌ لاحقٌ على `doc_type` أكّد أن لا ثالثةَ غيرَ `DOCUMENT_VALUE_LABEL`
// (وكانت مخطَّطةً في المواصفة د/٢). **والقائمةُ ليست عن «ما يُرحَّل» بل عن «ما
// له طرفٌ خارج الصالون»**، فدخولُها هنا لا يقترب من `post_stock_document`.
export const SUPPLIER_TYPES = ['supply', 'return_to_supplier', ORDER_DOC_TYPE]
...
//
// 🔴 THE LIST USED TO KEEP ONE THING OF ITS OWN — a checkbox for whether to
// look past the lens — because it was the only screen where "all storages" was
// a real question rather than an implicit choice.
//
// It is not the only one any more. The lens itself widens now, for exactly the
// two views that can mean it (catalog and documents), so the checkbox became a
// SECOND control answering a question the header above it already answered —
// the fault the catalogue's own picker had just been merged away for, sitting
// one screen over the whole time.
//
// ⇒ The state is gone and the function takes one argument. Re-exported from
// lib/storageScope.js because this file is inside the cycle that made a leaf
// module necessary.
export { storageInForce }
