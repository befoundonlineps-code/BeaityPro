...
// 🔴 A WINDOW call is not the aggregate this guard is about, and reading it as
// one made the guard bite correct SQL.
//
// `count(*) over ()` needs no `group by` and Postgres accepts it — so the
// failure this file exists to prevent (a rejected statement rolling back the
// DDL above it) cannot happen from it. The needle matched on `count(` alone and
// could not tell the two apart.
//
// ⚠️ **Measured, not reasoned:** 104_1 is a read-only catalogue query that uses
// `count(*) over ()` to report how many versions of a function exist. It was
// reported as a violation with 13 ungrouped columns beside it.
//
// ⇒ This NARROWS the needle and never widens it: an item is a bare aggregate
// only when no `over (` follows it. The 051c text that caused this guard still
// fails — proven by the counter-evidence below, which was not touched.
function isBareAggregate(item) {
  return AGGREGATES.test(item) && !/\bover\s*\(/i.test(item)
}
...
function violationsIn(sql) {
  ...
  for (const { text: statement } of statementsOf(sql)) {
    ...
    if (!items.some(isBareAggregate)) continue
...
describe('an aggregate does not sit beside an ungrouped column', () => {
  ...
  // 🔴 A WINDOW call beside ungrouped columns is legal, and the guard used to
  // report it. Both directions are pinned here so the narrowing cannot drift
  // into a hole: the window form passes, and DROPPING `over ()` from the very
  // same text brings the violation straight back.
  it('a window count is not the aggregate this guard is about — and without over() it bites again', () => {
    const windowed = `
      select
      ...
      from pg_proc p join pg_namespace n on n.oid = p.pronamespace
      where n.nspname = 'public' and p.proname like 'post_stocktake%';`

    // ⚠️ The counter-evidence is the SAME TEXT minus `over ()` — so it measures
    // the needle, not a second query that happens to differ elsewhere.
    const bare = windowed.replace('count(*) over ()', 'count(*)')

    expect(violationsIn(windowed)).toEqual([])
    expect(violationsIn(bare)).toHaveLength(2)
  })
