...
// Ticked for sale, with no price named. Returns which path it happened on —
// '', 'package', 'portion' or 'both'.
//
// ⚠️ THREE ANSWERS AND NOT A BOOLEAN, because a refusal names the state that
// occurred and not the class it belongs to. That rule was learned in this
// module on this exact shape: the fine fields are "both or neither", one
// message explained "a percentage with no basis", and the owner had done the
// opposite. The refusal was right and the reason shown was not his.
//
// ⚠️ BLANK, NEVER ZERO — the distinction the module is built on. Zero is a
// decision ("this one goes out free"); blank is an absence. Collapsing them is
// the same fault as `0 ₪` standing for an unknown cost.
//
// 🔴 AND IT COVERS BOTH PATHS, WHICH THE FIRST VERSION DID NOT. That version
// asked only about packages, on the reasoning that "the portions path has a
// completeness constraint and the packages path has none". That comparison was
// wrong — it put a DIVISOR beside a PRICE:
//
//     units_per_package   NOT NULL  default 1     ⇒ cannot go missing
//     units_per_portion   NULL                    ⇒ can, hence products_portion_check
//
//     package_price · portion_price · nominal_purchase_price
//                         all NULL OR >= 0        ⇒ silent on BOTH paths
//
// ⇒ On divisors the schema is symmetric and correct. On prices it says nothing
// either way, so the real question was never packages-versus-portions. It is
// whether "for sale" is allowed to mean "for sale with no price named" — and
// that question does not care which box was ticked.
//
// ⚠️ AND THE DEFAULTS ARE WHY IT IS URGENT rather than tidy:
//
//     sell_by_packages   NOT NULL  default TRUE   ⇒ opt-OUT, and unguarded
//     sell_by_portions   NOT NULL  default false  ⇒ opt-in
//
// So the loose state is not somewhere a person wanders by mistake. It is the
// state of every new row until somebody changes it.
export function missingSalePrice({
  sellByPackages, packagePrice, sellByPortions, portionPrice,
} = {}) {
  const blank = (value) => String(value ?? '').trim() === ''
  const noPackage = sellByPackages !== false && blank(packagePrice)
  const noPortion = !!sellByPortions && blank(portionPrice)

  if (noPackage && noPortion) return 'both'
  if (noPackage) return 'package'
  if (noPortion) return 'portion'
  return ''
}
...
// 080_1: every price column on products is `NULL OR >= 0` — package_price,
// portion_price, nominal_purchase_price alike. The schema has no opinion about
// a product offered for sale with no price named, and neither did this file.
//
...
export function productSaveAction({
  ...
  sellByPackages, packagePrice, sellByPortions, portionPrice, priceConfirmed,
}) {
  ...
  const missing = missingSalePrice({ sellByPackages, packagePrice, sellByPortions, portionPrice })
  if (missing && !priceConfirmed) return 'confirmNoPrice'
  return drop
}
...
//
// 🔴 `storage_id` IS DERIVED AND CARRIED, NEVER TYPED. There is no storage field
// in the folder dialog and there is not going to be one: the storage comes from
// the context the folder is created in — the parent, or the storage the screen
// is standing on — and where there is no context the creation is refused rather
// than guessed. lib/folderStorageInherit.js decides it; this only carries it.
//
// ⚠️ AND IT IS OMITTED ON AN EDIT RATHER THAN SENT AS NULL. `undefined` keys
// are dropped by the client before the request, so an update that does not know
// the storage leaves the column alone — while `storage_id: null` would UNSET
// the folder's storage on every rename. That is the difference between «I have
// nothing to say about this column» and «make it empty», and they are one
// keystroke apart.
// 🔴 لا `storage_id` هنا إطلاقًا — والعمودُ نفسُه يُسقَط بـ٠٨٨.
//
// انتماءُ المجلّد لمستودعٍ صار صفوفًا في `storage_categories`، لأن المجلّد يمكن
// أن يكون في أكثر من مستودع (وهو شرطُ النقل بين اثنين). فالحمولةُ رجعت حقلين
// كما كانت قبل ٠٨٥، **والانتماءُ يُكتب بنداءٍ ثانٍ** — `linkFolderToStorages`.
//
// ⚠️ **وآليّتان متوازيتان لمفهومٍ واحدٍ هي العلّةُ التي دفعنا ثمنَها هنا
// بالذات:** بُني العمودُ بينما جدولُ الربط قائمٌ ومبذورٌ ومحروسٌ بمُشغِّل، ولم
// يُرَ. فبقاؤه بعد اليوم — ولو معطَّلًا — يترك جوابين للسؤال نفسه.
