...
// that rgb(228,228,236), rgb(228, 228, 236) and #e4e4ec are all one thing —
// three spellings would otherwise be three ways to smuggle it in.
const SPELLINGS = [
  /rgb\(\s*228\s*,\s*228\s*,\s*236\s*\)/gi,
  /#e4e4ec\b/gi,
]
...
describe('the "you type here" blue is used for one thing', () => {
  ...
  it('appears exactly once, at its definition', () => {
    ...
    expect(hits).toEqual([
      { file: path.join('styles', 'globals.css'), line: '--write: rgb(228, 228, 236);' },
    ])
  })
})
