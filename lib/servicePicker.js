...
export function pruneEmpty(tree) {
  return (tree || [])
    ...
    .filter((node) => (node.services || []).length > 0 || node.children.length > 0)
}
...
export function filterServiceTree(tree, query) {
  ...
  // Recursive for the same reason the tree is: a folder inside a folder is a
  // shape this can now be handed, and filtering only two levels would drop
  // every match below the second.
  const narrow = (nodes) => (nodes || []).map((node) => ({
    ...node,
    services: (node.services || []).filter(match),
    children: narrow(node.children),
  }))

  return pruneEmpty(narrow(tree))
}
