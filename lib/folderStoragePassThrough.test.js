...
const CATEGORIES = [
  { id: 'c-care', parent_id: null, name: 'العناية', sort_order: 1, is_active: true },
  { id: 'c-shampoo', parent_id: 'c-care', name: 'شامبو', sort_order: 1, is_active: true },
  { id: 'c-nails', parent_id: null, name: 'أظافر', sort_order: 2, is_active: true },
  { id: 'c-loose', parent_id: null, name: 'قديم', sort_order: 3, is_active: true },
]

// 🔴 الانتماءُ صفوفٌ في storage_categories لا عمودٌ على المجلّد. و«قديم» بلا
// رابطٍ إطلاقًا — الحالةُ التي كان يمثّلها العمودُ الفارغ.
const LINKS = [
  { id: 'l1', storage_id: HAIR, category_id: 'c-care' },
  { id: 'l2', storage_id: COSMO, category_id: 'c-shampoo' },
  { id: 'l3', storage_id: COSMO, category_id: 'c-nails' },
]
...
const render = (over) => renderToStaticMarkup(
  <ProductsBrowser
    salonId="s" balances={[]}
    directories={{ storages: STORAGES, suppliers: [], storageCategories: LINKS, reload: () => {} }}
    storageId={COSMO}
    catalogue={{
...
describe('which folders a storage owns, and which it merely draws', () => {
  it('calls a folder from another storage pass-through, and its own not', () => {
    ...
    expect(isPassThroughFolder(care, COSMO, LINKS)).toBe(true)
    expect(isPassThroughFolder(shampoo, COSMO, LINKS)).toBe(false)
    // The same folder, from its own storage, is an ordinary folder.
    expect(isPassThroughFolder(care, HAIR, LINKS)).toBe(false)
  })
  ...
  it('calls nothing pass-through under «all storages»', () => {
    ...
    for (const c of CATEGORIES) {
      expect(isPassThroughFolder(c, ALL_STORAGES, LINKS)).toBe(false)
      expect(isPassThroughFolder(c, null, LINKS)).toBe(false)
      expect(isPassThroughFolder(c, '', LINKS)).toBe(false)
    }
  })

  it('treats a folder no storage keeps as pass-through under a storage, not under all', () => {
    const loose = CATEGORIES[3]
    expect(isPassThroughFolder(loose, COSMO, LINKS)).toBe(true)
    expect(isPassThroughFolder(loose, ALL_STORAGES, LINKS)).toBe(false)
  })
  ...
  it('answers about a folder that is not drawn here at all', () => {
    ...
    expect(foldersForStorage(CATEGORIES, COSMO, LINKS).map((c) => c.id)).not.toContain('c-loose')
    expect(isPassThroughFolder(CATEGORIES[3], COSMO, LINKS)).toBe(true)
  })
})
