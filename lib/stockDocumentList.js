...
// expensive one.
//
// Σ(quantity_base × unit_cost), which is unambiguous whatever the display unit
// happens to say, because both factors are per base unit.
//
// The MAGNITUDE is returned, not the signed sum. A write-off's sum is negative
// and a minus sign inside an Arabic line is a neutral character between two
// directions (lib/timeRangeDirection.test.js) — the document type already
// carries the direction as a word, which is the rule the direction badge came
// from.
export function documentValue(movements, documentId, productsById) {
  ...
  for (const m of movementsOf(movements, documentId)) {
    ...
    priced = true
    signed += quantity * cost.base
  }
  // Not priced at all is a different thing from priced at zero, and only the
  // first should show nothing.
  return priced ? Math.abs(signed) : null
}
