...
export const DOCUMENT_VALUE_LABEL = {
  // 🔴 **والطلبيّةُ سعرٌ مطلوبٌ لا كلفةٌ مدفوعة** — لا شراءَ وقع ولا دفعةَ
  // وُلدت. **وكلمةٌ محايدةٌ («القيمة») كانت ستضع رقمَ اتّفاقٍ مقترَحٍ بجانب
  // أرقامِ مالٍ خرج فعلًا** — وهو عينُ الاصطدام الذي وُجدت هذه القائمةُ لأجله.
  order: 'products:documents.valueOrder',
...
// 🔴 **ويُقارَن اليومُ لا النصُّ الخامّ — وهذا شرطُ صحّةٍ بعد دخول الطلبيّات.**
//
// `order_date` تاريخٌ بلا وقت و`doc_date` طابعٌ زمنيّ، **و`localeCompare`
// بينهما يُرجع `-1` دائمًا على اليوم نفسِه** ⇒ فكلُّ طلبيّةٍ تنزل تحت كلِّ
// مستندٍ من نفس اليوم مهما كان وقتُها. ⚠️ **ولا انهيارَ ولا خطأ — ترتيبٌ يبدو
// معقولًا ويكذب في اليوم المشترك وحدَه**، وهو أصعبُ ما يُلاحَظ.
//
// ✅ **ولا يضيع شيءٌ بالقصّ:** `doc_date` منتصفُ ليلٍ دائمًا، **فالمقصوصُ
// أصفار** — والترتيبُ داخل اليوم يحسمه `created_at` كما كان.
export function sortDocuments(documents) {
  return [...(documents || [])].sort((a, b) => {
    // ⚠️ **و`documentDate` نفسُها لا نسخةٌ منها** — القصُّ إلى اليوم مكتوبٌ
    // مرّةً واحدةً في هذا الملفّ، **ونسخةٌ ثانيةٌ باسمٍ ثانٍ هي الصنفُ المسجَّلُ
    // مؤجَّلًا بأربع نسخٍ من «الفراغُ عدمٌ لا صفر».**
    const byDate = documentDate(b.doc_date).localeCompare(documentDate(a.doc_date))
