...
export function buildCategoryTree(categories, options = {}) {
  ...
  //
  // `seen` cannot currently fire, and the comment it replaces said it stopped
  // the stack from giving out on a cycle in parent_id. Measured: it does not,
  // because it cannot be reached. The walk starts at folders with no parent,
  // and every folder inside a cycle has a parent pointing back into that
  // cycle — so no cycle is reachable from any root. What a cycle actually
  // does is make its whole branch vanish from the screen with nothing to say
  // why, which is why lib/categoryVisibility.js refuses to create one.
  //
  // It stays anyway, cheap and inert, because the day the root rule widens —
  // "a folder whose parent is missing is also a root" is the obvious next
  // step — a cycle becomes reachable in the same edit, and this is already
  // here. What changed is the claim, not the code.
