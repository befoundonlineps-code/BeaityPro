...
// Whether a folder is anywhere in this tree, at any depth.
//
// A screen holds the selected folder as an id, and the tree it is drawn from
// can lose folders — a filter hides the archived ones, a reload arrives
// without one somebody else just took out. The id survives either, and then
// points at something nobody can see: the pane on the right still lists that
// folder's rows, the pane on the left shows nothing selected, and the toolbar
// buttons still act on it. That is the ADR-048 shape again — state naming
// something that is no longer on screen — arriving through a filter instead
// of through an unmount.
//
// Here rather than in the screen because "is this id in this tree" is a
// question with an answer, and rules kept inside components have twice now
// turned out to be untested and wrong.
export function treeContains(nodes, id) {
  if (!id) return false
