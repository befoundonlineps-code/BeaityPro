...
//
// parent_id and business_type are always both present, never omitted. Moving
// a root under a parent has to clear its type and moving a sub out has to set
// one, and a key left out of an update leaves the old value in place for the
// check constraint to reject — or worse, leaves a folder pointing at a parent
// it was meant to leave.
export async function saveCategory({ id, name, parentId, businessType, salonId }, client = supabase) {
  const payload = {
    name: (name || '').trim(),
    parent_id: parentId || null,
    business_type: parentId ? null : (businessType || null),
  }
