...
describe('🔴 الشاشةُ ترسم القرارَ فعلًا — لا المكتبةُ وحدَها', () => {
  ...
  it('🔴 والمَخرجُ مُسمًّى، ومربوطٌ بمقبض الصفحة لا بتاريخ المتصفّح', () => {
    ...
    //
    // 🔴 **والمدى حدُّ العنصر نفسِه، لا ميزانيّةَ حروف — وهذا تصويبٌ بعد عضّة.**
    //
    // كان `[\s\S]{0,900}` **فسقط يومَ أُضيفت خاصّيّةٌ إلى العنصر**: خرج
    // `onClose` من النافذة، **والكودُ سليمٌ تمامًا.** ⇒ مدًى أضيقُ من جملته،
    // **يعضّ ما لم يوضع له، ويدفع لرفع الرقم — وهو نفسُ العطل مؤجَّلًا.**
    //
    // ⚠️ **والاختبارُ الذي يليه في هذا الملفّ نفسِه يفعلها صحيحًا** («الحدّان
    // معًا، والحدُّ الأعلى مثبَّتٌ باسمه») — **فالقاعدةُ كانت مكتوبةً على بُعد
    // عشرين سطرًا ولم تُطبَّق هنا.**
    const page = strip(read('pages/products/index.js'))
    const open = page.indexOf('<StockDocumentsList')
    expect(`العنصرُ موجودٌ في الصفحة = ${open >= 0}`).toBe('العنصرُ موجودٌ في الصفحة = true')

    // الحدُّ الأعلى نهايةُ الوسم نفسِه — يكبر مع خصائصه ولا يحتاج رقمًا يُرفَع.
    const close = page.indexOf('/>', open)
    expect(`للوسم نهايةٌ = ${close > open}`).toBe('للوسم نهايةٌ = true')

    const element = page.slice(open, close)
    expect(`onClose داخل الوسم = ${element.includes('onClose={closeOperation}')}`)
      .toBe('onClose داخل الوسم = true')
  })
