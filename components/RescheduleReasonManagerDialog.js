...
// A reason that was ever used on a real reschedule is kept forever — it is
...
//
// Same shape as CancellationReasonManagerDialog, deliberately kept as its
// own copy rather than a shared abstraction for now — see the note in the
// PR description about consolidating the three near-identical lookup
// managers (acquisition sources, cancellation reasons, this one) later.
export default function RescheduleReasonManagerDialog({ open, onOpenChange, reasons, loading, onChanged, salonId }) {
  ...
  async function handleSaveAdd() {
    ...
    setError('')
    setSaving(true)
    const { error } = await supabase.from('reschedule_reasons').insert([{ name: nameInput.trim(), salon_id: salonId }])
    setSaving(false)
  ...
  async function handleSaveEdit() {
    ...
    setError('')
    setSaving(true)
    const { error } = await supabase.from('reschedule_reasons').update({ name: nameInput.trim() }).eq('id', selectedReason.id)
    setSaving(false)
    if (error) setError(error.message)
    else {
      setEditOpen(false)
  ...
  async function handleConfirmDelete() {
    ...
    setInUse(false)
    setSaving(true)
    const { data, error } = await supabase.from('reschedule_reasons').delete().eq('id', selectedReason.id).select()
    setSaving(false)
    if (error) {
      // 23503 = foreign_key_violation: some appointment still cites this
      // reason. Deleting the label under a real reschedule would leave
      // that record unexplained, so deactivating is offered instead.
      if (error.code === '23503') {
        setInUse(true)
        setError(t('appointments:rescheduleReasonManager.deleteInUseMessage', { name }))
      } else {
    ...
    if (!data || data.length === 0) {
      setError(t('appointments:rescheduleReasonManager.deleteNoRowsMessage'))
      return
    }
    setSelectedId(null)
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleMainOpenChange}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('appointments:rescheduleReasonManager.title')}</DialogTitle>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-1.5">
            {loading ? (
              ...
              <div className="text-sm text-muted-foreground">{t('appointments:rescheduleReasonManager.empty')}</div>
            ) : (
              reasons.map((r) => (
                <button
                  type="button"
                ...
                >
                  ...
                  {!r.is_active && (
                    <span className="rounded bg-muted px-1.5 py-0.5 text-[11px] text-muted-foreground">
                      {t('appointments:rescheduleReasonManager.inactiveBadge')}
                    </span>
                  )}
                </button>
              ))
          ...
          <DialogFooter className="sm:justify-between">
            <Button type="button" variant="outline" size="sm" onClick={openAdd}>
              {t('appointments:rescheduleReasonManager.addReasonButton')}
            </Button>
            <div className="flex flex-wrap justify-end gap-2">
              <Button type="button" variant="outline" size="sm" disabled={!selectedId} onClick={handleToggleActive}>
                {selectedReason?.is_active
                  ? t('appointments:rescheduleReasonManager.deactivateButton')
                  : t('appointments:rescheduleReasonManager.activateButton')}
              </Button>
              <Button type="button" variant="outline" size="sm" disabled={!selectedId} onClick={openEdit}>
                {t('appointments:rescheduleReasonManager.editButton')}
              </Button>
              <Button type="button" variant="outline" size="sm" disabled={!selectedId} onClick={openDeleteConfirm}>
                {t('appointments:rescheduleReasonManager.deleteButton')}
              </Button>
            </div>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('appointments:rescheduleReasonManager.addDialogTitle')}</DialogTitle>
          </DialogHeader>
          <Input
            placeholder={t('appointments:rescheduleReasonManager.namePlaceholder')}
            value={nameInput}
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('appointments:rescheduleReasonManager.editDialogTitle')}</DialogTitle>
          </DialogHeader>
          <Input
            placeholder={t('appointments:rescheduleReasonManager.namePlaceholder')}
            value={nameInput}
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={closeDeleteConfirm}>
        <DialogContent>
          {deleteSuccessName ? (
            <>
              <DialogHeader>
                <DialogTitle>{t('appointments:rescheduleReasonManager.deleteSuccessMessage', { name: deleteSuccessName })}</DialogTitle>
              </DialogHeader>
              <DialogFooter>
            ...
            <>
              <DialogHeader>
                <DialogTitle>
                  {inUse
                    ? error
                    : t('appointments:rescheduleReasonManager.deleteConfirmMessage', { name: selectedReason?.name || '' })}
                </DialogTitle>
              </DialogHeader>
              ...
              <DialogFooter>
                ...
                {inUse ? (
                  <Button disabled={saving} onClick={handleDeactivateFromDeleteDialog}>
                    {saving ? t('common:saving') : t('appointments:rescheduleReasonManager.deactivateInsteadButton')}
                  </Button>
                ) : (
                  <Button variant="destructive" disabled={saving} onClick={handleConfirmDelete}>
                    {saving ? t('common:saving') : t('appointments:rescheduleReasonManager.confirmDeleteButton')}
                  </Button>
                )}
              </DialogFooter>
            </>
